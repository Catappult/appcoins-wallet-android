...
class AppcoinsRewardsTest {
  ...
  @Mock lateinit var billing: Billing

  @Mock lateinit var remoteApi: RemoteRepository
  ...
  fun setUp() {
        ...
        .thenReturn(
            Single.just(
                com.appcoins.wallet.core.network.microservices.model.Transaction(
                    "123456789",
                    com.appcoins.wallet.core.network.microservices.model.Transaction.Status
                        .PROCESSING,
                    null,
                    null,
                    null,
    ...
    `when`(billing.getAppcoinsTransaction(UID, scheduler))
        .thenReturn(
            Single.just(
                TransactionCore(
                    UID,
    ...
    appcoinsRewards =
        AppcoinsRewards(
            BdsAppcoinsRewardsRepository(remoteApi),
  ...
  @Test
  fun transferCredits() {
    val test = appcoinsRewards.sendCredits("", PRICE, PACKAGE_NAME).test()
    scheduler.triggerActions()
  ...
  fun transferCreditsNetworkError() {
    `when`(
        ...
        .thenReturn(
            Single.error(
                HttpException(
                    Response.error<AppcoinsRewardsRepository.Status>(
                        400, ResponseBody.create(null, "")))))
    val test = appcoinsRewards.sendCredits("", PRICE, PACKAGE_NAME).test()
    scheduler.triggerActions()
    test.assertNoErrors().assertComplete().assertValue(AppcoinsRewardsRepository.Status.API_ERROR)
  }
