...
class Permissions(private val repository: Repository<String, ApplicationPermission>) {
  ...
  /**
   * @throws SecurityException when trying to update the permission list of an apk and signature
   *   doesn't match
   */
  ...
  fun grantPermission(
    ...
    getApplication(walletAddress, packageName, apkSignature)?.let {
      ...
      saveApplicationPermission(
          walletAddress, packageName, ApplicationPermission(it, oldPermissions.toList()))
    }
        ?: saveApplicationPermission(
            walletAddress,
            packageName,
  ...
  /**
   * @throws SecurityException when trying to get the permission list of an apk with a wrong
   *   signature
   */
  @Throws(SecurityException::class)
  fun getPermissions(
  ...
  private fun getApplication(
    ...
    if (application != null &&
        (application.packageName != packageName || application.apkSignature != apkSignature)) {
  ...
  private fun getApplicationPermission(walletAddress: String, packageName: String) =
      repository.getSync(getKey(walletAddress, packageName))

  private fun getKey(walletAddress: String, packageName: String) = walletAddress + packageName

  fun getPermissions(walletAddress: String): Observable<List<ApplicationPermission>> {
    return repository.all.flatMapSingle {
      Observable.fromIterable(it)
          .filter { permission -> permission.walletAddress == walletAddress }
          .toList()
    }
  }
}
