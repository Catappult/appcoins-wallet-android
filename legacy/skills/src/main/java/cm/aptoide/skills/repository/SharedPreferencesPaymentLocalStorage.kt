...
@Inject
constructor(private val preferences: EskillsPreferencesDataSource, private val mapper: Gson) :
    PaymentLocalStorage {
  companion object {
  ...
  override fun save(cachedPayment: CachedPayment) =
      preferences.savePayments(
          getKey(cachedPayment.ticket.walletAddress), mapper.toJson(cachedPayment))

  override fun get(walletAddress: WalletAddress): Single<CachedPayment> {
    return Single.fromCallable {
      return@fromCallable preferences.getStoredPayments(getKey(walletAddress))?.let {
        mapper.fromJson(it, CachedPayment::class.java)
      } ?: throw RuntimeException("Couldn't find any cached payment.")
    }
  }
