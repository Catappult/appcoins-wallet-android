...
constructor(
  ...
  companion object {
    ...
    const val RESULT_PACKAGE_NAME_NOT_SUPPORTED = 12
    const val RESULT_PACKAGE_VERSION_NOT_SUPPORTED = 13
  ...
  fun getRoom(
    ...
    return Single.just(ticket)
        ...
        .flatMapObservable {
          getTicketUpdates(ticket.ticketId, eskillsPaymentData.queueId).flatMap {
  ...
  private fun handlePaymentResultStatus(
    ...
    return when (paymentResult) {
      ...
      is FailedPayment.FraudError ->
          isWalletVerifiedUseCase().observeOn(AndroidSchedulers.mainThread()).doOnSuccess {
            view.showFraudError(it)
          }
    ...
    return when (ticket) {
      is CreatedTicket -> {
        return when (ticket.processingStatus) {
          ProcessingStatus.PENDING_PAYMENT ->
              Observable.just(UserData.fromStatus(UserData.Status.PAYING))
          ProcessingStatus.REFUNDED ->
              Observable.just(UserData.fromStatus(UserData.Status.REFUNDED))
          ProcessingStatus.IN_QUEUE,
          ProcessingStatus.REFUNDING ->
              Observable.just(UserData.fromStatus(UserData.Status.IN_QUEUE, ticket.queueId))
        }
      }
      is PurchasedTicket -> {
  ...
  private fun getTicketUpdates(
    ...
    return Observable.interval(GET_ROOM_RETRY_MILLIS, TimeUnit.MILLISECONDS).switchMapSingle {
      getTicketUseCase(ticketId, queueIdentifier)
    }
  }

  fun cancelPayment() {
    closeView.onNext(Pair(RESULT_USER_CANCELED, UserData.fromStatus(UserData.Status.REFUNDED)))
  }
  ...
  fun cancelTicket(): Single<TicketResponse?> {
    ...
    return cancelTicketUseCase(ticketId)
        ...
        .doOnError {
          closeView.onNext(Pair(RESULT_ERROR, UserData.fromStatus(UserData.Status.REFUNDED)))
        }
  }
  ...
  fun restorePurchase(view: PaymentView): Single<Ticket> {
    return walletAddressObtainer.getWalletAddress().flatMap { walletAddress ->
