...
class PeriodicGameChecker(
  ...
  fun start(session: String) {
    disposables.add(
        Observable.interval(0, getRoomPeriodSeconds, TimeUnit.SECONDS)
            .flatMapSingle<Any> {
              roomRepository
                  .getRoom(session)
                  .observeOn(AndroidSchedulers.mainThread())
                  .doOnSuccess { roomResponse: RoomResponse ->
                    checkGameStatus(roomResponse as RoomResponse.SuccessfulRoomResponse)
                  }
            }
  ...
  private fun checkGameStatus(roomResponse: RoomResponse.SuccessfulRoomResponse) {
    when (roomResponse.status) {
      ...
      RoomStatus.COMPLETED ->
          gameStateListener.onFinishGame(
  ...
  private fun isWinner(roomResponse: RoomResponse.SuccessfulRoomResponse): Boolean {
    return roomResponse.roomResult.winner.walletAddress.equals(
        roomResponse.currentUser.walletAddress, ignoreCase = true)
  }
