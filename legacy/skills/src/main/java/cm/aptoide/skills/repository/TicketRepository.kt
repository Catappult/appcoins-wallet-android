...
constructor(
  ...
  fun createTicket(
    ...
    return ticketApi
        .postTicket(ewt, buildTicketRequest(eskillsPaymentData, walletAddress))
  ...
  fun postReferralTransaction(ewt: String, referralCode: String): Single<ReferralResult> {
    return ticketApi
        .postReferralTransaction(ewt, referralCode)
        .subscribeOn(Schedulers.io())
        .map { SuccessfulReferral(it) as ReferralResult }
        .onErrorReturn { referralMapper.mapHttpException(it as HttpException) }
  }

  fun getFirstTimeUserCheck(ewt: String): Single<Boolean> {
    return ticketApi
        .getFirstTimeUserCheck(ewt)
        .subscribeOn(Schedulers.io())
  ...
  fun getReferral(ewt: String): Single<ReferralResponse> {
    return ticketApi.getReferral(ewt).subscribeOn(Schedulers.io())
  }

  fun createReferral(ewt: String): Single<ReferralResponse> {
    return ticketApi.createReferral(ewt).subscribeOn(Schedulers.io())
  }
}
