...
constructor(
  ...
  operator fun invoke(eskillsPaymentData: EskillsPaymentData): Single<Ticket> {
    return walletAddressObtainer
        ...
        .flatMap { walletAddress ->
          ewtObtainer.getEWT().flatMap { ewt ->
            ticketRepository.getInQueueTicket(walletAddress, eskillsPaymentData).flatMap {
        ...
        .doOnSuccess { ticket: Ticket ->
          if (ticket is CreatedTicket) {
            ticketRepository.cacheTicket(ticket.walletAddress, ticket.ticketId, eskillsPaymentData)
          }
        }
  }
  ...
  private fun createOrResumeTicket(
    ...
    return when (storedTicket) {
      EmptyStoredTicket -> ticketRepository.createTicket(eskillsPaymentData, ewt, walletAddress)
      is StoredTicketInQueue ->
          resumeTicketIfPossible(ewt, storedTicket, eskillsPaymentData, walletAddress)
    }
  }
  ...
  private fun resumeTicketIfPossible(
    ...
    return ticketRepository
        .getTicket(ewt, ticketInQueue.ticketId, eskillsPaymentData.queueId)
        .flatMap {
        ...
        .onErrorResumeNext { ticketRepository.createTicket(eskillsPaymentData, ewt, walletAddress) }
  }
}
