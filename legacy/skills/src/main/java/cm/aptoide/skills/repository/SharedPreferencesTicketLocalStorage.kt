...
constructor(private val preferences: EskillsPreferencesDataSource, private val mapper: Gson) :
  ...
  override fun getTicketInQueue(
    ...
    return Single.fromCallable {
      ...
      if (data != null && data == eskillsPaymentData) {
        return@fromCallable preferences.getTicketInQueue(getWalletKey(walletAddress))?.let {
          StoredTicketInQueue(it)
        } ?: EmptyStoredTicket
      }
  ...
  private fun getData(walletAddress: WalletAddress): EskillsPaymentData? {
    return preferences.getTicketData(getDataKey(walletAddress))?.let {
      mapper.fromJson(it, EskillsPaymentData::class.java)
    }
  }
