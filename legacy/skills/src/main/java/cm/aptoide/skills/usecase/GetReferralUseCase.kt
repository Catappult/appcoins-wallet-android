...
constructor(
  ...
  operator fun invoke(): Single<ReferralResponse> {
    return ewtObtainer.getEWT().flatMap { ewt ->
      ticketRepository.getReferral(ewt).onErrorReturn {
        return@onErrorReturn if (it.getErrorCodeAndMessage().first == 404)
            ticketRepository.createReferral(ewt).onErrorReturn { failedReferral }.blockingGet()
        else failedReferral
      }
    }
  }
