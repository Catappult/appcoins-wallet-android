...
class GamificationTest {
  ...
  fun getUserStats() {
    ...
    val testObserver = gamification.getUserStats(WALLET, null).test()
  ...
  fun getUserLevelWithLevelIncreasing() {
    ...
    api.userStatusResponse =
        Single.just(
            UserStatusResponse(
                listOf(userStatsGamification, referralResponse), WalletOrigin.APTOIDE))
    val testObserver = gamification.getUserLevel(WALLET, null).test()
        ...
        Single.just(
            listOf(
                GamificationResponse(
                    ...
                    PromotionsResponse.Status.ACTIVE,
                    false)))
    api.userStatusResponse = Single.just(UserStatusResponse(emptyList(), WalletOrigin.PARTNER))
    val testObserver = gamification.getUserLevel(WALLET, null).test()
    ...
    api.levelsResponse =
        Single.just(
            LevelsResponse(
                ...
                LevelsResponse.Status.ACTIVE,
                date))
    val testObserver = gamification.getLevels(WALLET).test()
    testObserver.assertResult(
        Levels(
            Levels.Status.OK,
    ...
    api.levelsResponse =
        Single.just(
            LevelsResponse(
                ...
                LevelsResponse.Status.ACTIVE,
                date))
    val testObserver = gamification.getLevels(WALLET, false).test()
  ...
  fun getBonus() {
    ...
    val testObserver =
        gamification.getEarningBonus(WALLET, PACKAGE_NAME, BigDecimal.ONE, "", null).test()
  ...
  fun hasNewLevelNoNewLevel() {
    ...
    val test =
        gamification
            .hasNewLevel(WALLET, GamificationContext.SCREEN_MY_LEVEL, userStatsGamification.level)
            .test()
  ...
  fun levelShown() {
    val shownLevel = 1
    val test =
        gamification.levelShown(WALLET, shownLevel, GamificationContext.SCREEN_MY_LEVEL).test()
