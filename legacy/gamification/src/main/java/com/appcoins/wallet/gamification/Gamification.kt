...
class Gamification @Inject constructor(private val repository: PromotionsRepository) {
  ...
  fun getUserBonusAndLevel(
    ...
    return repository
        .getUserStats(wallet, promoCodeString, false)
        .map { map(it) }
        .lastOrError()
  ...
  private fun map(userStats: UserStats): ForecastBonusAndLevel {
    val gamification =
        userStats.promotions.firstOrNull { it is GamificationResponse && it.id == GAMIFICATION_ID }
            as GamificationResponse?

    val referral =
        userStats.promotions.firstOrNull { it is GamificationResponse && it.id == REFERRAL_ID }
            as ReferralResponse?

    return if (referral == null || referral.pendingAmount.compareTo(BigDecimal.ZERO) == 0) {
      ForecastBonusAndLevel(
          status = ForecastBonus.Status.INACTIVE, level = gamification?.level ?: 0)
    } else {
      ForecastBonusAndLevel(
  ...
  fun hasNewLevel(
    ...
    return repository.getLastShownLevel(wallet, gamificationContext).map { lastShownLevel: Int ->
