  ...
  companion object {
    ...
    val MIGRATION_3_4: Migration =
        object : Migration(3, 4) {
          override fun migrate(database: SupportSQLiteDatabase) {
            // Since this involves changing the table structure (primary key), 4 steps need to be
            // done:
        ...
        object : Migration(7, 8) {
          override fun migrate(database: SupportSQLiteDatabase) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
              database.execSQL(
                  "ALTER TABLE PromotionEntity RENAME COLUMN gamification_type to gamification_status")
            }
          }
        }
    ...
    val MIGRATION_8_9: Migration =
        object : Migration(8, 9) {
          override fun migrate(database: SupportSQLiteDatabase) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
              database.execSQL(
                  "ALTER TABLE PromotionEntity RENAME COLUMN gamification_status to gamification_type")
            }
          }
        }
