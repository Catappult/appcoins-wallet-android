...
@Inject
constructor(private val api: GamificationApi, private val local: UserStatsLocalData) :
  ...
  private fun getUserStatsFromDB(
      ...
      Single.zip(local.getPromotions(), local.retrieveWalletOrigin(wallet)) {
          ...
          .onErrorReturn { mapErrorToUserStatsModel(throwable ?: it, throwable == null) }
  ...
  private fun getUserStatsFromAPI(
      ...
      api.getUserStats(wallet, Locale.getDefault().language, promoCodeString)
          ...
          .flatMapObservable {
            local
                .deleteAndInsertPromotions(it.promotions)
  ...
  override fun getGamificationStats(
      ...
      getUserStatsFromResponses(wallet, promoCodeString).map {
  ...
  private fun getLevelsFromAPI(wallet: String, useDbOnError: Boolean = false): Observable<Levels> =
      api.getLevels(wallet)
          .flatMapObservable {
            local.deleteLevels().andThen(local.insertLevels(it)).toSingle { map(it) }.toObservable()
          }
          .onErrorResumeNext { throwable: Throwable ->
  ...
  override fun getUserStats(
      ...
      getUserStatsFromResponses(wallet, promoCodeString, offlineFirst)
          .flatMap { userStatusResponse ->
            val gamification =
                userStatusResponse.promotions.firstOrNull { it is GamificationResponse }
  ...
  override fun getReferralUserStatus(
      ...
      getUserStatsFromResponses(wallet, promoCodeString, false)
          .lastOrError()
          .flatMap {
            val gamification =
                it.promotions.firstOrNull { promotions -> promotions is GamificationResponse }
                    as GamificationResponse?
            val referral =
                it.promotions.firstOrNull { promotions -> promotions is ReferralResponse }
                    as ReferralResponse?
            if (gamification != null) {
  ...
  override fun isReferralNotificationToShow(wallet: String): Observable<Boolean> {
    return getGamificationStats(wallet, null)
        .subscribeOn(Schedulers.io())
        .map {
          (!local.isReferralNotificationSeen(wallet)) &&
              it.gamificationStatus == GamificationStatus.VIP
        }
        .onErrorReturn { false }
  }
  ...
  private fun isNoNetworkException(throwable: Throwable): Boolean =
      throwable is IOException || throwable.cause != null && throwable.cause is IOException

  override fun getVipReferral(wallet: String): Single<VipReferralResponse> =
      api.getVipReferral(wallet).subscribeOn(Schedulers.io()).onErrorReturn {
