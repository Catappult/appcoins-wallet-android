...
constructor(
  ...
  override fun getGamificationLevel() = preferences.getGamificationLevel(INVALID_LEVEL)
  ...
  private fun getKeyGeneric(screen: String, id: String) = SHOWN_GENERIC + SCREEN + screen + ID + id

  override fun getPromotions(): Single<List<PromotionsResponse>> {
    return promotionDao.getPromotions().map { filterByDate(it) }.map { mapToPromotionResponse(it) }
  }
  ...
  override fun deleteAndInsertPromotions(promotions: List<PromotionsResponse>): Completable {
    return Single.create<List<PromotionEntity>> { emitter ->
          val results: List<PromotionEntity> =
              promotions.map {
                when (it) {
  ...
  override fun deleteLevels(): Completable {
    return levelDao.deleteLevels().andThen(levelsDao.deleteLevels())
  }
  ...
  override fun insertLevels(levels: LevelsResponse): Completable {
    ...
    val levelEntityList = levels.list.map { LevelEntity(null, it.amount, it.bonus, it.level) }
  ...
  override fun retrieveWalletOrigin(wallet: String): Single<WalletOrigin> {
    return walletOriginDao.getWalletOrigin(wallet).map { it.walletOrigin }
  }
  ...
  override fun isVipCalloutAlreadySeen(wallet: String) = preferences.isVipCalloutAlreadySeen(wallet)
