...
class AppcoinsBillingBinder(
    private val supportedApiVersion: Int,
  ...
  override fun getSkuDetails(
    ...
    return try {
      val serializedProducts =
          billing
              .getProducts(merchantName, skus, type)
  ...
  override fun getBuyIntent(
    ...
    val getSkuDetails =
        billing.getProducts(merchantName, listOf(sku), type).subscribeOn(networkScheduler)
    return Single.zip(getTokenContractAddress, getIabContractAddress, getSkuDetails) {
          ...
          try {
            ...
            if (type == BillingSupportedType.INAPP_SUBSCRIPTION &&
  ...
  override fun getPurchases(
    ...
    try {
      ...
      val purchases = billing.getPurchases(merchantName, type).blockingGet()
  ...
  override fun consumePurchase(apiVersion: Int, packageName: String?, purchaseToken: String): Int {
    ...
    return try {
      billing.consumePurchases(purchaseToken, merchantName).map { RESULT_OK }.blockingGet()
    } catch (exception: Exception) {
