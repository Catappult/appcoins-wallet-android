...
constructor(
  ...
  fun loadPaymentInfo(
    ...
    return adyenApi
        .loadPaymentInfo(walletAddress, ewt, value, currency, methods.transactionType)
  ...
  fun makePayment(
    ...
    val shopperInteraction =
        if (!hasCvc && supportedShopperInteractions.contains("ContAuth")) {
          "ContAuth"
        } else "Ecommerce"
    return if (transactionType == BillingSupportedType.INAPP_SUBSCRIPTION.name) {
      ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMap { ewt
        -> // keep both auths for this one
        subscriptionsApi
            .getSkuSubscriptionToken(
                domain = packageName!!,
                sku = sku!!,
                currency = currency,
            ...
            .flatMap {
              adyenApi.makeTokenPayment(
                  walletAddress = walletAddress, authorization = ewt, payment = it)
            }
  ...
  fun disablePayments(walletAddress: String): Single<Boolean> {
    return adyenApi
        .disablePayments(DisableWallet(walletAddress))
