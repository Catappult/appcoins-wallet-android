...
class AdyenPaymentRepositoryTest {
  ...
  @Mock lateinit var adyenApi: AdyenApi

  @Mock lateinit var brokerBdsApi: BrokerBdsApi

  @Mock lateinit var subscriptionsApi: SubscriptionBillingApi

  @Mock lateinit var mapper: AdyenResponseMapper
        ...
        EwtAuthenticatorService(
            object : WalletService {
              ...
              override fun findWalletOrCreate(): Observable<String> =
                  Observable.just(TEST_WALLET_ADDRESS)
              ...
              override fun getAndSignSpecificWalletAddress(
                  walletAddress: String
  ...
  fun loadPaymentInfoTest() {
    ...
    val model = PaymentInfoModel(null, false, BigDecimal(2), TEST_FIAT_CURRENCY)
    Mockito.`when`(
    ...
    testObserver.assertNoErrors().assertValue { it == model }
  }
  ...
  fun loadPaymentInfoErrorTest() {
    ...
    Mockito.`when`(mapper.mapInfoModelError(throwable)).thenReturn(model)
    val testObserver = TestObserver<PaymentInfoModel>()
  ...
  fun disablePaymentTest() {
    Mockito.`when`(adyenApi.disablePayments(DisableWallet(TEST_WALLET_ADDRESS)))
        .thenReturn(Completable.complete())
    val testObserver = TestObserver<Boolean>()
    adyenRepo.disablePayments(TEST_WALLET_ADDRESS).subscribe(testObserver)
  ...
  // TEST_EWT,
  //        TokenPayment(modelObject, false, "", "Ecommerce", null, null, null, paymentType, null,
  // null,
