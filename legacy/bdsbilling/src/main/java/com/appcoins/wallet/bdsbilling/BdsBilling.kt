...
class BdsBilling(
  ...
  override fun isInAppSupported(merchantName: String): Single<Billing.BillingSupportType> {
    return repository
        .isSupported(merchantName, BillingSupportedType.INAPP)
        .map { map(it) }
  ...
  override fun isSubsSupported(merchantName: String): Single<Billing.BillingSupportType> {
    return repository
        .isSupported(merchantName, BillingSupportedType.INAPP_SUBSCRIPTION)
        .map { map(it) }
        .onErrorReturn { errorMapper.map(it) }
  }
  ...
  override fun getAppcoinsTransaction(uid: String, scheduler: Scheduler): Single<Transaction> {
    return walletService.getAndSignCurrentWalletAddress().observeOn(scheduler).flatMap {
      repository.getAppcoinsTransaction(uid, it.address, it.signedAddress)
    }
  }
  ...
  override fun getSkuPurchase(
    ...
    return walletService.getAndSignCurrentWalletAddress().observeOn(scheduler).flatMap {
      repository.getSkuPurchase(merchantName, sku, purchaseUid, it.address, it.signedAddress, type)
    }
  }
  ...
  override fun getPurchases(
    ...
    return if (isManagedType(type)) {
      walletService
          ...
          .observeOn(scheduler)
          .flatMap { repository.getPurchases(packageName, it.address, it.signedAddress, type) }
  ...
  override fun consumePurchases(
    ...
    return repository.consumePurchases(merchantName, purchaseToken, type).observeOn(scheduler)
  }
  ...
  override fun getSubscriptionToken(
    ...
    return walletService.getAndSignCurrentWalletAddress().observeOn(networkThread).flatMap {
  ...
  override fun getPaymentMethods(
    ...
    return partnerAddressService.getAttribution(packageName).flatMap { attributionEntity ->
      walletService.getWalletAddress().flatMap { address ->
  ...
  private fun map(it: Boolean) =
      if (it) Billing.BillingSupportType.SUPPORTED
      else Billing.BillingSupportType.MERCHANT_NOT_FOUND
}
