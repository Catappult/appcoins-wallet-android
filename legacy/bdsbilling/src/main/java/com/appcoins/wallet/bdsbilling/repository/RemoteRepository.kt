...
class RemoteRepository(
  ...
  internal fun getSkuPurchase(packageName: String, skuId: String?): Single<Purchase> =
      ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMap { ewt ->
        inappApi
            .getPurchases(
                packageName = packageName,
                authorization = ewt,
              ...
              if (it.items.isEmpty()) {
                throw HttpException(
                    Response.error<GetPurchasesResponse>(
                        404, ResponseBody.create("application/json".toMediaType(), "{}")))
              }
  ...
  internal fun acknowledgePurchase(
      ...
      ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMap { ewt ->
        inappApi
            .acknowledgePurchase(domain = packageName, uid = purchaseToken, authorization = ewt)
            .toSingle { true }
      }
  ...
  internal fun consumePurchase(
      ...
      ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMap { ewt ->
        inappApi
            .consumePurchase(domain = packageName, uid = purchaseToken, authorization = ewt)
            .toSingle { true }
      }
  ...
  fun registerPaymentProof(
      ...
      ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMapCompletable { ewt ->
  ...
  fun getAppcoinsTransaction(
      ...
      brokerBdsApi.getAppcoinsTransaction(
          uId = uid, walletAddress = address, walletSignature = signedContent)
