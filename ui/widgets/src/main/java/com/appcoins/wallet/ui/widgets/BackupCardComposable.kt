...
fun BackupAlertCard(onClickButton: () -> Unit, hasBackup: Boolean, backupDate: Long = 0L) {
  AlertCard(
      ...
      title =
          if (hasBackup) R.string.backup_confirmation_no_share_title
          else R.string.my_wallets_action_backup_wallet,
      message =
          if (hasBackup)
              stringResource(
      ...
      buttonLabel =
          if (hasBackup) R.string.mywallet_backup_again_button else R.string.action_backup_wallet,
...
fun VerifyWalletAlertCard(onClickButton: () -> Unit, verified: Boolean) {
  AlertCard(
      onClickButton = onClickButton,
      title =
          if (verified) R.string.verification_settings_verified_title
...
fun AlertCard(onClickButton: () -> Unit, title: Int, message: String, buttonLabel: Int, icon: Int) {
  Column {
    AlertMessageWithIcon(icon = icon, title = stringResource(id = title), message = message)
