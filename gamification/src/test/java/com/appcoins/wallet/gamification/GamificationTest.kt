...
class GamificationTest {
  ...
  fun hasNewLevelNoNewLevel() {
    ...
    val test = gamification.hasNewLevel(WALLET, GamificationContext.SCREEN_MY_LEVEL.toString())
        .test()
  ...
  fun levelShown() {
    val shownLevel = 1
    val test = gamification.levelShown(WALLET, shownLevel, GamificationContext.SCREEN_MY_LEVEL.toString())
        .test()
    test.assertNoErrors()
        .assertComplete()
