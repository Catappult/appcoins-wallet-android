...
@RunWith(AndroidJUnit4.class) public class WalletRepoTest {
  ...
  static final String STORE_1 = "{\"address\":\"eb1a948c6cc57fedf9271626404fc04a74ddd1e6\","
  ...
  @Test public void testCreateAccount() {
    TestObserver<Wallet> subscription = accountRepository.createWallet(PASS_1)
        .test();
    subscription.awaitTerminalEvent();
    subscription.assertComplete();
    assertEquals(subscription.valueCount(), 1);
    deleteAccount(subscription.values()
        .get(0).address, PASS_1);
  }
  ...
  @Test public void testImportAccount() {
    ...
    Assert.assertEquals(subscriber.values()
        .get(0).address, ADDRESS_1);
    Assert.assertTrue(subscriber.values()
        .get(0)
  ...
  @Test public void testDeleteAccount() {
    ...
    TestObserver<Void> subscriber = accountRepository.deleteWallet(ADDRESS_1, PASS_1)
        .test();
    subscriber.awaitTerminalEvent();
    subscriber.assertComplete();
    ...
    Assert.assertEquals(accountListSubscriber.values()
        .get(0).length, 0);
  }
  ...
  @Test public void testExportAccountStore() {
    ...
    TestObserver<String> subscriber =
        accountRepository.exportWallet(new Wallet(ADDRESS_1), PASS_1, PASS_1)
            .test();
    subscriber.awaitTerminalEvent();
    subscriber.assertComplete();
    Assert.assertEquals(subscriber.valueCount(), 1);
  ...
  @Test public void testFetchAccounts() {
    ...
    TestObserver<Wallet[]> subscriber = accountRepository.fetchWallets()
        .test();
    subscriber.awaitTerminalEvent();
    subscriber.assertComplete();
    Assert.assertEquals(subscriber.valueCount(), 1);
    ...
    for (int i = 0; i < 100; i++) {
      Assert.assertTrue(createdWallets.get(i)
  ...
  @Test public void testFindAccount() {
    importAccount(STORE_1, PASS_1);
    TestObserver<Wallet> subscribe = accountRepository.findWallet(ADDRESS_1)
        .test();
    subscribe.awaitTerminalEvent();
    subscribe.assertComplete();
    assertEquals(subscribe.valueCount(), 1);
    assertTrue(subscribe.values()
        .get(0)
        .sameAddress(ADDRESS_1));
    deleteAccount(ADDRESS_1, PASS_1);
  }

  @Test public void testSetDefaultAccount() {
    Wallet wallet = createAccount();
    TestObserver<Void> subscriber = accountRepository.setDefaultWallet(wallet)
        .test();
    subscriber.awaitTerminalEvent();
    subscriber.assertComplete();
    ...
    assertTrue(defaultSubscriber.values()
        .get(0)
  ...
  @Test public void testGetBalance() {
    ...
    subscriber.awaitTerminalEvent();
    subscriber.assertComplete();
    Log.d("BALANCE", subscriber.values()
        .get(0)
        .toString());
    deleteAccount(ADDRESS_1, PASS_1);
  }
  ...
  private Wallet createAccount() {
    ...
    accountRepository.createWallet("1234")
        .toObservable()
