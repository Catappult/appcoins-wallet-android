...
@HiltViewModel
class OnboardingPaymentMethodsViewModel
@Inject
constructor(
    private val getFirstPaymentMethodsUseCase: GetFirstPaymentMethodsUseCase,
    private val getOtherPaymentMethodsUseCase:
        GetOtherPaymentMethodsUseCase, // temporary, to remove later and use
    // getFirstPaymentMethodsUseCase only
    ...
    cachedTransactionRepository
        ...
        .flatMap { cachedTransaction ->
          ...
          if (cachedTransaction.value <= 0.0) {
            diffCachedTransaction = cachedTransaction.copy(value = args.amount.toDouble())
          }
          if (cachedTransaction.currency.isNullOrEmpty()) {
            diffCachedTransaction = diffCachedTransaction.copy(currency = args.currency)
          }
          getFirstPaymentMethodsUseCase(diffCachedTransaction)
              // to only use getFirstPaymentMethodsUseCase and remove the doOnSuccess after all
              // methods are ready
              ...
              .asAsyncToState { copy(paymentMethodsAsync = it) }
        }
  ...
  fun handleBackToGameClick() {
    ...
    sendSideEffect {
      OnboardingPaymentMethodsSideEffect.NavigateBackToGame(args.transactionBuilder.domain)
    }
  }
}
