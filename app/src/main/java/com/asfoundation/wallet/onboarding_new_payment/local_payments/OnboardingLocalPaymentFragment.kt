...
@AndroidEntryPoint
class OnboardingLocalPaymentFragment :
  ...
  @Inject lateinit var navigator: OnboardingLocalPaymentNavigator
  ...
  override fun onStateChanged(state: OnboardingLocalPaymentState) {
    when (state.transaction) {
      ...
      is Async.Success -> {
        state.transaction.value?.url?.let { navigator.navigateToWebView(it, webViewLauncher) }
      }
      is Async.Fail -> {
        showError(null)
      }
  ...
  override fun onSideEffect(sideEffect: OnboardingLocalPaymentSideEffect) {
    when (sideEffect) {
      is OnboardingLocalPaymentSideEffect.NavigateToWebView -> {
        hideLoading()
        navigator.navigateToWebView(sideEffect.uri, webViewLauncher)
      }
      OnboardingLocalPaymentSideEffect.NavigateBackToPaymentMethods ->
  ...
  private fun clickListeners() {
    binding.errorView.errorDismiss.setOnClickListener { viewModel.handleBackButton() }
