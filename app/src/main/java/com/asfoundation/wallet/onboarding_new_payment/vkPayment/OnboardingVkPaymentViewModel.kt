...
@HiltViewModel
class OnboardingVkPaymentViewModel
@Inject
constructor(
  ...
  fun getPaymentLink(email: String, phone: String) {
    ...
    createVkPayTransactionUseCase(
        ...
        .doOnSuccess { sendSideEffect { OnboardingVkPaymentSideEffect.PaymentLinkSuccess } }
        .scopedSubscribe()
  }
            ...
            when (it.status) {
              ...
              Transaction.Status.FRAUD -> {
                ...
                events.sendAdyenPaymentConfirmationEvent(
                    args.transactionBuilder, BillingAnalytics.ACTION_CANCEL, PaymentType.VKPAY.name)
                sendSideEffect { OnboardingVkPaymentSideEffect.ShowError }
              }
  ...
  fun handleBackToGameClick() {
    ...
    sendSideEffect {
      OnboardingVkPaymentSideEffect.NavigateBackToGame(args.transactionBuilder.domain)
    }
  }
