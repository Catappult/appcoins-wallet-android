...
class GetFirstPaymentMethodsUseCase
@Inject
constructor(
  ...
  operator fun invoke(cachedTransaction: CachedTransaction): Single<List<PaymentMethod>> {
    return partnerAddressService
        .getAttribution(packageName = cachedTransaction.packageName ?: "")
        .flatMap { attributionEntity ->
          bdsRepository
              ...
              .flatMap { paymentMethods ->
                removeUnavailableMethods(paymentMethods).flatMap { availablePaymentMethods ->
  ...
  /**
   * Since this is the first payment, we won't need to show appcoins credits nor appcoins as a
   * payment option, unlike the payment in the normal IAB flow, where we hide the option if no
   * balance is available. In this case, we won't show it at all.
   */
  private fun removeUnavailableMethods(
      paymentList: List<PaymentMethodEntity>
    ...
    while (iterator.hasNext()) {
      ...
      if (method.id == CREDITS_ID ||
          method.id == APPC_ID ||
          method.id == CHALLENGE_REWARD_ID ||
          !method.isAvailable()) {
        iterator.remove()
      }
    }
