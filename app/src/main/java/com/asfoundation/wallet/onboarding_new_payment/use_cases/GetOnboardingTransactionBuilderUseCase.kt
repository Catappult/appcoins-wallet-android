...
class GetOnboardingTransactionBuilderUseCase
@Inject
constructor(
  ...
  operator fun invoke(cachedTransaction: CachedTransaction): Single<TransactionBuilder> {
    return Single.just(parseOneStep(Uri.parse(cachedTransaction.referrerUrl))).flatMap { oneStepUri
      ->
      cachedTransaction.referrerUrl?.let { referrerUrl ->
        oneStepTransactionParser.buildTransaction(oneStepUri, referrerUrl)
      }
    }
  }
}
