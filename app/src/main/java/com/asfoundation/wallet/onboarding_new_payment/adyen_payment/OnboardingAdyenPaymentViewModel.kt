...
data class OnboardingAdyenPaymentState(
    val paymentInfoModel: Async<PaymentInfoModel> = Async.Uninitialized
) : ViewState

@HiltViewModel
class OnboardingAdyenPaymentViewModel
@Inject
constructor(
  ...
  private fun handlePaymentInfo() {
    events.sendPaymentMethodEvent(
        args.transactionBuilder, args.paymentType, BillingAnalytics.ACTION_BUY)
    getPaymentInfoModelUseCase(
            paymentType = args.paymentType.toString(),
            value = args.amount,
            currency = args.currency)
        .asAsyncToState { copy(paymentInfoModel = it) }
        .scopedSubscribe()
  }
  ...
  fun handleBuyClick(adyenCard: AdyenCardWrapper, shouldStoreCard: Boolean, returnUrl: String) {
    ...
    transactionOriginUseCase(args.transactionBuilder)
        ...
        .doOnSuccess { paymentModel -> handlePaymentResult(paymentModel) }
        .scopedSubscribe()
  }
    ...
    when {
      paymentModel.refusalReason != null -> {
        paymentModel.refusalCode?.let { code ->
          when (code) {
            AdyenErrorCodeMapper.CVC_DECLINED ->
                sendSideEffect { OnboardingAdyenPaymentSideEffect.ShowCvvError }
            else ->
                sendSideEffect {
      ...
      paymentModel.status == PaymentModel.Status.PENDING_USER_PAYMENT &&
          paymentModel.action != null -> {
  ...
  fun handleBackButton() {
    events.sendPaymentMethodEvent(
        args.transactionBuilder, args.paymentType, BillingAnalytics.ACTION_BACK)
      ...
      WebViewActivity.FAIL -> {
        when {
          result.data?.dataString?.contains("codapayments") != true -> {
            if (result.data
                ?.dataString
                ?.contains(BillingWebViewFragment.CARRIER_BILLING_ONE_BIP_SCHEMA) == true) {
              events.sendCarrierBillingConfirmationEvent(
                  args.transactionBuilder, BillingAnalytics.ACTION_CANCEL)
            } else {
