...
@AndroidEntryPoint
class OnboardingPaymentMethodsFragment :
  ...
  @Inject lateinit var navigator: OnboardingPaymentMethodsNavigator
  ...
  private fun handlePaymentMethodList() {
    ...
    controller.clickListener = { paymentMethodClick ->
      when (paymentMethodClick) {
        is PaymentMethodClick.CreditCardClick ->
            navigator.navigateToAdyen(
                args.transactionBuilder, args.amount, args.currency, args.forecastBonus)
        is PaymentMethodClick.PaypalAdyenClick ->
            navigator.navigateToPaypalAdyen(
                args.transactionBuilder, args.amount, args.currency, args.forecastBonus)
        is PaymentMethodClick.PaypalDirectClick ->
            navigator.navigateToPaypalAdyen(
                args.transactionBuilder, args.amount, args.currency, args.forecastBonus)
        is PaymentMethodClick.LocalPaymentClick ->
            navigator.navigateToLocalPayment(
                args.transactionBuilder, paymentMethodClick.idItem, args.amount, args.currency)
        ...
        is PaymentMethodClick.VkPayPaymentClick ->
            navigator.navigateToVkPayPayment(
                args.transactionBuilder, args.amount, args.currency, args.forecastBonus)
        is PaymentMethodClick.GooglePayClick ->
            navigator.navigateToGooglePay(
                args.transactionBuilder, args.amount, args.currency, args.forecastBonus)
  ...
  override fun onSideEffect(sideEffect: OnboardingPaymentMethodsSideEffect) {
    when (sideEffect) {
      is OnboardingPaymentMethodsSideEffect.NavigateToLink ->
          navigator.navigateToBrowser(sideEffect.uri)
      is OnboardingPaymentMethodsSideEffect.NavigateBackToGame ->
  ...
  /**
   * setStringWithLinks() and setLinkToString() is duplicated from the OnboardingFragment and should
   * be extracted to a extension function to avoid this. The action calls the viewModel and this
   * should be set as an argument (the action) to be able to extract this logic for usage in both
   * places
   */
  private fun setStringWithLinks() {
    ...
    val termsPolicyTickBox =
        resources.getString(
            R.string.agree_by_choosing_a_payment_method_body, termsConditions, privacyPolicy)
