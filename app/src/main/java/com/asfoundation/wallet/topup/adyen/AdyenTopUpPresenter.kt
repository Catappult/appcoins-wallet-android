...
class AdyenTopUpPresenter(
  ...
  private fun handleViewState() {
    if (currentError != 0) {
      ...
      if (waitingResult) view.showLoading() else loadPaymentMethodInfo()
    }
  }
        ...
        convertAmount()
            .flatMap {
              adyenPaymentInteractor.loadPaymentInfo(
                  mapPaymentToService(paymentType), it.toString(), currency)
            }
                ...
                if (it.error.isNetworkError) view.showNetworkError()
                    ...
                    handleSpecificError(
                        ...
                        it.error.errorInfo?.run {
                          Exception(
                              "PaymentMethodInfo type=$errorType code=$httpCode mCode=$messageCode")
  ...
  private fun handleForgetCardClick() {
    disposables.add(
        view
            ...
            .flatMapSingle {
              adyenPaymentInteractor
                  .loadPaymentInfo(mapPaymentToService(paymentType), amount, currency)
                  .observeOn(viewScheduler)
                  .doOnSuccess {
                    view.hideLoading()
                    if (it.error.hasError) {
                      if (it.error.isNetworkError) view.showNetworkError()
                        ...
                        handleSpecificError(
                            R.string.unknown_error,
                            logMessage =
                                "Message: ${it.error.errorInfo?.text}, code: ${it.error.errorInfo?.httpCode}")
                      }
                    } else {
  ...
  /**
   * A function needs to be created due to a problem with adyen not enabling the CVC in the same
   * fragment as it was disabled so we need to disable the payment and recreate the fragment
   */
  private fun handleForgetStoredCardClick() {
    disposables.add(
        view
            .forgetStoredCardClick()
            .observeOn(viewScheduler)
            .doOnNext { view.showLoading() }
            .observeOn(networkScheduler)
            .flatMapSingle { adyenPaymentInteractor.disablePayments() }
        ...
        view
            ...
            .flatMapSingle {
              adyenPaymentInteractor.submitRedirect(
                  cachedUid, convertToJson(it.details!!), it.paymentData ?: cachedPaymentData)
            }
            .observeOn(viewScheduler)
            ...
            paymentModel.refusalCode?.let { code ->
              when (code) {
                ...
                FRAUD -> {
                  ...
                  riskRules = paymentModel.fraudResultIds.sorted().joinToString(separator = "-")
                }
                else ->
                    handleSpecificError(
                        adyenErrorCodeMapper.map(code),
                        Exception(
                            "PaymentResult paymentType=$paymentType code=$code reason=${paymentModel.refusalReason}"))
              }
            }
        ...
        retrieveFailedReason(paymentModel.uid)
          ...
          Completable.fromAction {
            ...
            handleSpecificError(
                R.string.unknown_error,
                Exception(
                    "PaymentResult paymentType=$paymentType code=${paymentModel.refusalCode}"))
          }
    }
  }
    ...
    return adyenPaymentInteractor
        ...
        .flatMapCompletable {
          Completable.fromAction {
            ...
            val message =
                if (it.errorCode != null) adyenErrorCodeMapper.map(it.errorCode!!)
                else R.string.unknown_error
            handleSpecificError(
                message,
                Exception(
                    "FailedReason code=${it.errorCode} paymentType=$paymentType status=${it.status}"))
          }
        }
  }
            ...
            .doOnSuccess { verified ->
              if (verified) {
                ...
                handleSpecificError(
                    fraudError,
                    Exception(
                        "FraudFlow paymentMethod=$paymentMethodRuleBroken amount=$amountRuleBroken"))
              } else {
  ...
  private fun handleCreditCardNeedCVC() {
    disposables.add(
        adyenPaymentInteractor
            .getCreditCardNeedCVC()
            .observeOn(viewScheduler)
            .doOnSuccess { view.handleCreditCardNeedCVC(it.needAskCvc) }
            .doOnError { view.handleCreditCardNeedCVC(true) }
            .subscribe())
  }
  ...
  private fun convertAmount(): Single<BigDecimal> {
    return if (selectedCurrency == TopUpData.FIAT_CURRENCY) {
      Single.just(BigDecimal(amount))
        ...
        adyenPaymentInteractor
            .convertToLocalFiat(BigDecimal(appcValue).toDouble())
  ...
  private fun createBundle(
    ...
    return billingMessagesMapper.topUpBundle(
        priceAmount, priceCurrency, bonusValue.toPlainString(), fiatCurrencySymbol)
  }
    ...
    when {
      ...
      paymentModel.error.errorInfo?.errorType == ErrorType.INVALID_CARD -> {
        logger.log(
            TAG,
            Exception(
                "Errors paymentType=$paymentType type=${paymentModel.error.errorInfo?.errorType} code=${paymentModel.error.errorInfo?.httpCode}"))
      ...
      paymentModel.error.errorInfo?.httpCode != null -> {
        ...
        if (paymentModel.error.errorInfo?.errorType == ErrorType.BLOCKED)
            handleFraudFlow(resId, emptyList())
        else {
          logger.log(
              TAG,
              Exception(
