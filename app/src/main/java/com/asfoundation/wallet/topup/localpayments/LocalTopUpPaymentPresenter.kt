...
class LocalTopUpPaymentPresenter(
  ...
  private fun setupUi() {
    ...
    view.showValues(
        fiatAmount,
        data.topUpData.fiatCurrencyCode,
        appcAmount,
        ...
        localPaymentInteractor
            ...
            .doOnSuccess {
              analytics.sendConfirmationEvent(
                  data.topUpData.appcValue.toDouble(), "top_up", data.paymentId)
  ...
  private fun handleTransactionStatus(transaction: Transaction): Completable {
    return when {
      ...
      data.async ->
          Completable.fromAction {
                analytics.sendSuccessEvent(
                    data.topUpData.appcValue.toDouble(), data.paymentId, "pending")
              }
  ...
  private fun handleSupportClicks() {
    disposables.add(
        Observable.merge(view.getSupportIconClicks(), view.getSupportLogoClicks())
            .throttleFirst(50, TimeUnit.MILLISECONDS)
            .flatMapCompletable {
              localPaymentInteractor.showSupport(data.topUpData.gamificationLevel)
            }
            .subscribe({}, { it.printStackTrace() }))
  }
  ...
  private fun createBundle(
    ...
    return localPaymentInteractor.topUpBundle(
        priceAmount, priceCurrency, data.topUpData.bonusValue.toPlainString(), fiatCurrencySymbol)
  }
