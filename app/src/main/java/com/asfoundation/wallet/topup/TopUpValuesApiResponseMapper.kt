...
class TopUpValuesApiResponseMapper @Inject constructor() {
  ...
  fun map(defaultValues: TopUpDefaultValuesResponse): TopUpValuesModel {
    return TopUpValuesModel(
        ArrayList(
            defaultValues.items.map {
              FiatValue(
                  BigDecimal(it.price.fiat.value),
                  it.price.fiat.currency.code,
  ...
  fun mapValues(limitValuesResponse: TopUpLimitValuesResponse): TopUpLimitValues {
    return TopUpLimitValues(
        FiatValue(
            BigDecimal(limitValuesResponse.values.min),
            limitValuesResponse.currency.code,
            limitValuesResponse.currency.sign),
        FiatValue(
            BigDecimal(limitValuesResponse.values.max),
            limitValuesResponse.currency.code,
            limitValuesResponse.currency.sign))
  }
}
