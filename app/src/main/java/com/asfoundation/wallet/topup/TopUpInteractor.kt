...
constructor(
  ...
  fun getLimitTopUpValues(): Single<TopUpLimitValues> =
      if (limitValues.maxValue != TopUpLimitValues.INITIAL_LIMIT_VALUE &&
        ...
        topUpValuesService.getLimitValues().doOnSuccess {
          if (!it.error.hasError) cacheLimitValues(it)
        }
      }
  ...
  fun getDefaultValues(): Single<TopUpValuesModel> =
      if (chipValueIndexMap.isNotEmpty()) {
        ...
        topUpValuesService.getDefaultValues().doOnSuccess {
          if (!it.error.hasError) cacheChipValues(it.values)
        }
      }
