...
data class VkPaymentTopUpState(val vkTransaction: Async<VkPayTransaction> = Async.Uninitialized) :
    ViewState

@HiltViewModel
class VkPaymentTopUpViewModel
@Inject
constructor(
...
) : BaseViewModel<VkPaymentTopUpState, VkPaymentTopUpSideEffect>(VkPaymentTopUpState()) {
  ...
  fun getPaymentLink(email: String, phone: String) {
    if (isFirstGetPaymentLink) {
      ...
      getCurrentWalletUseCase().doOnSuccess { walletAddress = it.address }.scopedSubscribe()
      val price = VkPrice(value = paymentData.fiatValue, currency = paymentData.fiatCurrencySymbol)
      createVkPayTransactionTopUpUseCase(price = price, email = email, phone = phone)
          .asAsyncToState { copy(vkTransaction = it) }
          .doOnSuccess { sendSideEffect { VkPaymentTopUpSideEffect.PaymentLinkSuccess } }
          .scopedSubscribe()
    }
  }
            ...
            when (it.status) {
              ...
              Transaction.Status.FRAUD -> {
                ...
                analytics.sendErrorEvent(
                    paymentData.appcValue.toDouble(), "top_up", PaymentType.VKPAY.name, "", "")
                sendSideEffect {
                  VkPaymentTopUpSideEffect.ShowError(R.string.purchase_error_wallet_block_code_403)
                }
              }
