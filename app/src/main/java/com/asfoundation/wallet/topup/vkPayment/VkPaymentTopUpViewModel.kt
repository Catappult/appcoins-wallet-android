...
data class VkPaymentTopUpState(
    ...
    val vkTransactionDev: Async<Boolean> = Async.Uninitialized
) : ViewState
...
class VkPaymentTopUpViewModel @Inject constructor(
    ...
    private val changeVkPayTransactionStatusDevUseCase: ChangeVkPayTransactionStatusDevUseCase,
    ...
    fun changeVkTransactionStatusDev() {
        changeVkPayTransactionStatusDevUseCase(
            status = null
        ).asAsyncToState {
            copy(vkTransactionDev = it)
        }.scopedSubscribe()
    }
