...
data class OnboardingPaymentState(
    val transactionContent: Async<TransactionContent> = Async.Uninitialized
) : ViewState
...
constructor(
  ...
  fun handleContent() {
    cachedTransactionRepository
        ...
        .flatMap { cachedTransaction ->
          Single.zip(
              ...
              bdsRepository.getSkuDetails(
                  ...
                  BillingSupportedType.INAPP)) { transactionBuilder, products ->
        ...
        .flatMap { (cachedTransaction, transactionBuilder, products) ->
          ...
          val modifiedTransactionBuilder =
              transactionBuilder.amount(
                  BigDecimal(products.first().transactionPrice.appcoinsAmount))
        ...
        .asAsyncToState { copy(transactionContent = it) }
        .doOnSuccess { sendSideEffect { OnboardingPaymentSideEffect.ShowPaymentMethods(it) } }
        .scopedSubscribe()
  }
