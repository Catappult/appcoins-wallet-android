...
constructor(
  ...
  fun parse(uri: String): Single<TransactionBuilder> {
    if (uri.isEthereumURLString()) {
      return Single.just(parseERC681(uri)).flatMap { erc681 ->
        eipTransactionParser.buildTransaction(erc681)
      }
    } else if (Uri.parse(uri).isOneStepURLString()) {
      return Single.just(parseOneStep(Uri.parse(uri))).flatMap { oneStepUri ->
        oneStepTransactionParser.buildTransaction(oneStepUri, uri)
      }
    }
