...
data class Period(val years: Int, val months: Int, val weeks: Int, val days: Int) : Serializable {
  ...
  companion object {
     ...
     *
     * This will parse the string produced by `toString()` which is based on the ISO-8601 period
     * formats `PnYnMnD` and `PnW`.
     *
     ...
     * four sections must be present. The sections have suffixes in ASCII of "Y", "M", "W" and "D"
     * for years, months, weeks and days, accepted in upper or lower case. The suffixes must occur
     * in order. The number part of each section must consist of ASCII digits. The number may be
     ...
     * standard. In addition, ISO-8601 does not permit mixing between the `PnYnMnD` and `PnW`
