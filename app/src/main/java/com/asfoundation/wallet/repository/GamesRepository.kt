...
class GamesRepository @Inject constructor(private val gamesApi: GamesApi) : GamesRepositoryType {

  override fun getGamesListing(): Single<List<GameData>> {
    return gamesApi.getGamesListing(Locale.getDefault().language).map {
      it.map {
        GameData(
            title = it.appName,
            gameIcon = it.appIcon,
