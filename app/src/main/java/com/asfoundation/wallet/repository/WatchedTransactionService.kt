...
class WatchedTransactionService(
  ...
  private fun executeTransaction(watchedTransaction: WatchedTransaction): Completable {
    return cache
        ...
        .observeOn(scheduler)
        .andThen(
            transactionSender.send(watchedTransaction.transactionBuilder).flatMapCompletable { hash
              ->
              cache
                  .save(
                      watchedTransaction.key,
        ...
        .doOnError {
          ...
          cache.saveSync(
              ...
              WatchedTransaction(
                  watchedTransaction.key,
                  enumValueOf(paymentErrorMapper.map(it).paymentState.name),
                  watchedTransaction.transactionBuilder))
        }
  }
  ...
  fun sendTransaction(key: String, transactionBuilder: TransactionBuilder): Completable {
    return cache.save(
        key, WatchedTransaction(key, WatchedTransaction.Status.PENDING, transactionBuilder))
  }

  fun getTransaction(key: String): Observable<WatchedTransaction> =
      cache.get(key).filter { it.status != WatchedTransaction.Status.PENDING }
