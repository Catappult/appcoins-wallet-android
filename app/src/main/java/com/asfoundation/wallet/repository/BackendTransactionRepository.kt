...
constructor(
    ...
    return Observable.fromIterable(transactions)
        .flatMapSingle { transaction ->
          if (isRevertTransaction(transaction.type)) {
            localRepository
                .getRevertedTransaction(wallet, transaction.transactionId)
                ...
                .onErrorResumeNext {
                  ...
                  localRepository
                      .getRevertedTxId(wallet, transaction.transactionId)
                      .flatMap { link ->
                        offChainTransactions
                            .getTransactionsById(wallet, link)
            ...
            localRepository
                .getRevertTransaction(wallet, transaction.transactionId)
                .map { link -> mapper.map(transaction, link) }
                .onErrorReturn { mapper.map(transaction) }
          }
        }
  ...
  private fun fetchMissingOldTransactions(
    ...
    return localRepository.isOldTransactionsLoaded().flatMapObservable { isLoaded ->
      if (isLoaded) {
        return@flatMapObservable Observable.empty<MutableList<WalletHistory.Transaction>>()
      }
      return@flatMapObservable localRepository
          .getOlderTransaction(wallet)
          .map { it.processedTime }
  ...
  private fun fetchTransactions(
    ...
    return TransactionsLoadObservable(offChainTransactions, wallet, startingDate, endDate, sort)
        .flatMapSingle { transactions -> Observable.fromIterable(transactions).toList() }
  }
  ...
  private fun getLastProcessedTime(wallet: String): Maybe<Long> {
    ...
    return if (lastLocale == null || lastLocale == currentLocale) {
      ...
      localRepository.getNewestTransaction(wallet).map { it.processedTime }.defaultIfEmpty(0)
    } else {
      Maybe.fromCallable {
