...
import com.asfoundation.wallet.repository.entity.RealmToken;
import com.asfoundation.wallet.repository.entity.RealmTokenTicker;
...
public class TokensRealmSource implements TokenLocalSource {
    ...
    return Completable.fromAction(() -> {
      ...
      try {
        ...
        for (TokenTicker tokenTicker : tokenTickers) {
          ...
          realmItem.setImage(
              TextUtils.isEmpty(tokenTicker.image) ? String.format(COINMARKETCAP_IMAGE_URL,
                  tokenTicker.id) : tokenTicker.image);
    ...
    return Single.fromCallable(() -> {
      ...
      try {
        ...
        for (int i = 0; i < len; i++) {
          ...
          if (rawItem != null) {
            tokenTickers.add(
                new TokenTicker(rawItem.getId(), rawItem.getContract(), rawItem.getPrice(),
                    rawItem.getPercentChange24h(), rawItem.getImage()));
          }
        }
        realm.commitTransaction();
      } finally {
      ...
      return tokenTickers.size() == 0 ? null
  ...
  private Token[] convert(RealmResults<RealmToken> realmItems, long now) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (realmItem != null) {
        TokenInfo info =
            new TokenInfo(realmItem.getAddress(), realmItem.getName(), realmItem.getSymbol(),
                realmItem.getDecimals(), realmItem.getEnabled(), realmItem.getAddedManually());
        BigDecimal balance = TextUtils.isEmpty(realmItem.getBalance())
            || realmItem.getUpdatedTime() + ACTUAL_BALANCE_INTERVAL < now ? null
            : new BigDecimal(realmItem.getBalance());
