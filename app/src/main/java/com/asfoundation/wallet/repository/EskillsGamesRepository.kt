...
class EskillsGamesRepository @Inject constructor(
  ...
  override fun getGamesListing(): Single<List<GameData>> {
    return gamesApi.getGamesListing(
      ...
      .map {
        it.dataList.list.map { gameInfo ->
          GameData(
            title = gameInfo.appName,
            gameIcon = gameInfo.appIcon,
            gameBackground = gameInfo.background,
            gamePackage = gameInfo.packageName
          )
        }
      }
  }
