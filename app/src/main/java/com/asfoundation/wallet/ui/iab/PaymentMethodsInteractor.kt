...
constructor(
  ...
  fun resume(
      ...
      inAppPurchaseInteractor.resume(
          uri,
          transactionType,
          packageName,
          productName,
  ...
  fun getPaymentMethods(
      ...
      inAppPurchaseInteractor.getPaymentMethods(transaction, transactionValue, currency).map {
        filterValidGooglePayUseCase(it)
      }
  ...
  fun getSkuPurchase(
    ...
    return inAppPurchaseInteractor.getCompletedPurchaseBundle(
        type, appPackage, skuId, purchaseUid, orderReference, hash, networkThread)
  }

  fun getSkuDetails(domain: String, sku: String, type: BillingSupportedType): Single<Product> {
    return billing.getProducts(domain, mutableListOf(sku), type).map { products ->
      products.first()
    }
  }
  ...
  fun isAbleToSubscribe(
    ...
    return billing
        .getSubscriptionToken(packageName, skuId, networkThread)
