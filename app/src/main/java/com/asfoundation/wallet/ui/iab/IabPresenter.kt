...
class IabPresenter(
  ...
  fun present(savedInstanceState: Bundle?) {
    savedInstanceState?.let { firstImpression = it.getBoolean(FIRST_IMPRESSION, firstImpression) }
              ...
              if (firstImpression) {
                if (iabInteract.hasPreSelectedPaymentMethod()) {
                  billingAnalytics.sendPurchaseStartEvent(
                      ...
                      iabInteract.getPreSelectedPaymentMethod(),
                      transaction?.type,
    ...
    when (resultCode) {
      WebViewActivity.FAIL -> {
        if (data?.dataString?.contains("codapayments") != true) {
          if (data?.dataString?.contains(BillingWebViewFragment.CARRIER_BILLING_ONE_BIP_SCHEMA) ==
              true) {
