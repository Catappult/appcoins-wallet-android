...
class MergedAppcoinsPresenter(
  ...
  private fun fetchBalance() {
    resumeDisposables.add(
        getWalletInfoUseCase(null, cached = true)
            .map { walletInfo ->
            ...
            .doOnSuccess { balance ->
              val appcFiat =
                  formatter.formatPaymentCurrency(
                      balance.appcFiatValue.amount, WalletCurrency.APPCOINS)
              val creditsFiat =
                  formatter.formatPaymentCurrency(
                      balance.creditsFiatBalance.amount, WalletCurrency.CREDITS)
            ...
            .flatMap {
              ...
              Single.zip(
                  ...
                  mergedAppcoinsInteractor.retrieveAppcAvailability(
                      transactionBuilder, isSubscription)) {
                      hasCredits: Availability,
                      hasAppc: Availability ->
  ...
  private fun handleBackClick() {
    disposables.add(
        Observable.merge(view.backClick(), view.backPressed())
            .observeOn(networkScheduler)
            .doOnNext { paymentMethod ->
              analytics.sendPaymentConfirmationEvent(
                  paymentMethod.packageName,
                  paymentMethod.skuDetails,
                  paymentMethod.value,
                  paymentMethod.purchaseDetails,
  ...
  private fun navigateToPayment(selectedPaymentId: String) {
    when (paymentMethodsMapper.map(selectedPaymentId)) {
      PaymentMethodsView.SelectedPaymentMethod.APPC ->
          view.navigateToAppcPayment(transactionBuilder)
      PaymentMethodsView.SelectedPaymentMethod.APPC_CREDITS ->
          view.navigateToCreditsPayment(transactionBuilder)
      else -> {
