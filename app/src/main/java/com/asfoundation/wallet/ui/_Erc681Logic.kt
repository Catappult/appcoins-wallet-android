...
class _Erc681Logic(
  ...
  fun invoke(data: String, callerPackage: String?, productName: String) {
    if (data.contains("/buy?")) {
      navigator.navigate(_StartSend(data, callerPackage, productName))
    } else {
        ...
        .flatMap {
          parse(data)
            .map { transactionBuilder ->
              ...
              if (callingPackage == null) callingPackage = callerPackage
            ...
            .flatMap { transactionBuilder ->
              isWalletFromBds(transactionBuilder.domain, transactionBuilder.toAddress())
                .doOnSuccess { navigator.navigate(_StartTransfer(transactionBuilder, it)) }
            }
            .toObservable()

        }
        .subscribe({ }, {
          logger.log("Erc681Receiver", it)
  ...
  private fun parse(data: String) = if (data.isEthereumURLString()) {
