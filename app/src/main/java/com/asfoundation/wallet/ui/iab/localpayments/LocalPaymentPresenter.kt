...
class LocalPaymentPresenter(
  ...
  fun present(savedInstance: Bundle?) {
    ...
    savedInstance?.let { waitingResult = savedInstance.getBoolean(WAITING_RESULT) }
  ...
  fun preparePendingUserPayment() {
    disposables.add(
        Single.zip(getPaymentMethodIcon(), getApplicationIcon()) {
  ...
  private fun getPaymentMethodIcon() =
      Single.fromCallable {
  ...
  private fun handlePaymentRedirect() {
    disposables.add(
        navigator
            ...
            .flatMap {
              localPaymentInteractor.getTransaction(it, data.async).subscribeOn(networkScheduler)
            }
            .observeOn(viewScheduler)
  ...
  private fun handleOkErrorButtonClick() {
    disposables.add(
        view
            .getErrorDismissClick()
            .doOnNext { view.dismissError() }
            .subscribe({}, { view.dismissError() }))
  }

  private fun handleOkBuyButtonClick() {
    disposables.add(view.getGotItClick().doOnNext { view.close() }.subscribe({}, { view.close() }))
  }
  ...
  private fun handleFraudFlow() {
    disposables.add(
        localPaymentInteractor
            ...
            .flatMap { blocked ->
              if (blocked) {
                localPaymentInteractor.isWalletVerified().observeOn(viewScheduler).doOnSuccess {
                  if (it) {
  ...
  private fun handleTransactionStatus(transaction: Transaction): Completable {
    ...
    return when {
      ...
      data.async ->
          // Although this should no longer happen at the moment in Iab, since it doesn't consume
          // much process time
          // I decided to leave this here in case the API wants to change the logic and return them
          // to Iab in the future.
  ...
  private fun handleSyncCompletedStatus(transaction: Transaction): Completable {
    return localPaymentInteractor
        .getCompletePurchaseBundle(
            ...
            data.packageName,
            data.skuId,
            transaction.metadata?.purchaseUid,
        ...
        .flatMapCompletable {
          Completable.fromAction { view.showCompletedPayment() }
              .andThen(
                  Completable.timer(
                      view.getAnimationDuration(), TimeUnit.MILLISECONDS, viewScheduler))
      ...
      Status.PENDING_USER_PAYMENT -> {
        Completable.fromAction {
          analytics.sendPendingPaymentEvents(
              data.packageName, data.skuId, data.appcAmount.toString(), data.type, data.paymentId)
        }
      }
      Status.COMPLETED -> {
        Completable.fromAction {
  ...
  private fun getWidth(): Int {
    return TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_PX, 184f, context?.resources?.displayMetrics)
        .toInt()
  }

  private fun getHeight(): Int {
    return TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_PX, 80f, context?.resources?.displayMetrics)
        .toInt()
  }
  ...
  private val appIconWidth: Int
    get() =
        TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_PX, 160f, context?.resources?.displayMetrics)
            .toInt()
