...
class WalletRemoveConfirmationPresenter(
    private val view: WalletRemoveConfirmationView,
  ...
  private fun handleNoButtonClick() {
    disposable.add(
        view.noButtonClick().observeOn(viewScheduler).doOnNext { view.navigateBack() }.subscribe())
  }
  ...
  private fun deleteWallet(): Single<Unit> {
    return Single.zip(
            deleteWalletInteract.delete(walletAddress).toSingleDefault(Unit),
            Completable.timer(2, TimeUnit.SECONDS).toSingleDefault(Unit),
