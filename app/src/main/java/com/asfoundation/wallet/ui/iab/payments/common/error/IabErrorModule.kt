...
class IabErrorModule {
  ...
  fun providesIabErrorData(fragment: Fragment): IabErrorData {
    fragment.requireArguments().apply {
      val errorMessage =
          if (containsKey(IabErrorFragment.ERROR_MESSAGE_STRING)) {
  ...
  @Provides
  fun providesIabErrorPresenter(
      fragment: Fragment,
      data: IabErrorData,
      navigator: IabErrorNavigator,
      supportInteractor: SupportInteractor
  ): IabErrorPresenter {
    return IabErrorPresenter(
        fragment as IabErrorView, data, navigator, supportInteractor, CompositeDisposable())
  }
}
