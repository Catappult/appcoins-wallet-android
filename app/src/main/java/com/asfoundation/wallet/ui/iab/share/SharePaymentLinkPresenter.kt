...
class SharePaymentLinkPresenter(
        ...
        view
            ...
            .flatMapSingle {
              analytics.sendPaymentConfirmationEvent(
                  it.domain, it.skuId ?: "", it.amount, it.paymentMethod, it.type, "share")
              getLink(it)
            }
            .observeOn(viewScheduler)
            .doOnNext {
              interactor.savePreSelectedPaymentMethod(
                  PaymentMethodsView.PaymentMethodId.ASK_FRIEND.id)
              view.shareLink(it)
            }
            .subscribe(
                {},
  ...
  private fun handleStop() {
    disposables.add(
        view
            .getCancelButtonClick()
            .doOnNext {
              analytics.sendPaymentConfirmationEvent(
                  it.domain, it.skuId ?: "", it.amount, it.paymentMethod, it.type, "close")
              view.close()
            }
            .subscribe())
  }
