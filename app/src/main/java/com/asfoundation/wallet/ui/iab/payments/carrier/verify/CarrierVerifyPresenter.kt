...
class CarrierVerifyPresenter(
  ...
  private fun initializeView() {
    view.initializeView(
        data.currency, data.fiatAmount, data.skuDescription, data.bonusAmount, data.preselected)
    disposables.add(
        appInfoProvider
            .getApplicationInfo(data.domain)
            .observeOn(viewScheduler)
            .doOnSuccess { ai -> view.setAppDetails(ai.appName, ai.icon) }
  ...
  private fun formatFiatValue(value: BigDecimal, currencyCode: String): String {
    ...
    var scaledBonus =
        value.stripTrailingZeros().setScale(CurrencyFormatUtils.FIAT_SCALE, BigDecimal.ROUND_DOWN)
