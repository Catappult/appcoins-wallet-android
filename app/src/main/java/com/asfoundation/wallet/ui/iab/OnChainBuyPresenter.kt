...
class OnChainBuyPresenter(
              ...
              onChainBuyInteract
                  ...
                  .flatMapCompletable { currentPaymentStep: CurrentPaymentStep ->
                    when (currentPaymentStep) {
                      CurrentPaymentStep.PAUSED_ON_CHAIN ->
                          onChainBuyInteract.resume(
                              uriString,
                              AsfInAppPurchaseInteractor.TransactionType.NORMAL,
                              appPackage,
                              transaction.skuId,
                              ...
                              transaction.type,
                              transactionBuilder)
                      CurrentPaymentStep.READY ->
                          Completable.fromAction { setup(appcAmount) }.subscribeOn(viewScheduler)
                      CurrentPaymentStep.NO_FUNDS ->
  ...
  private fun showError(throwable: Throwable?, message: String? = null, userMessage: Int? = null) {
    ...
    if (throwable is UnknownTokenException) view.showWrongNetworkError() else view.showError()
  }
  ...
  private fun showPendingTransaction(transaction: Payment): Completable {
    ...
    return when (transaction.status) {
      Payment.Status.COMPLETED -> {
        ...
        onChainBuyInteract
            ...
            .flatMapCompletable { bundle ->
              handleSuccessTransaction(bundle, transaction.uid ?: "")
            }
            .onErrorResumeNext { Completable.fromAction { showError(it) } }
      }
      Payment.Status.NO_FUNDS ->
          Completable.fromAction {
      ...
      Payment.Status.FORBIDDEN ->
          Completable.fromAction { handleFraudFlow() }
              .andThen(onChainBuyInteract.remove(transaction.uri))
      Payment.Status.SUB_ALREADY_OWNED ->
          Completable.fromAction {
            showError(null, "Sub Already Owned", R.string.subscriptions_error_already_subscribed)
          }
      Payment.Status.ERROR ->
          Completable.fromAction { showError(null, "Payment status: ${transaction.status.name}") }
              .andThen(onChainBuyInteract.remove(transaction.uri))
      else ->
          Completable.fromAction { showError(null, "Payment status: UNKNOWN") }
              .andThen(onChainBuyInteract.remove(transaction.uri))
    }
  }
  ...
  private fun buildBundle(payment: Payment, orderReference: String?): Bundle {
    return if (payment.uid != null && payment.signature != null && payment.signatureData != null) {
      billingMessagesMapper.mapPurchase(
          payment.uid!!, payment.signature!!, payment.signatureData!!, orderReference)
    } else {
      Bundle().also {
    ...
    if (isError(status)) {
      if (payment.errorCode == null && payment.errorMessage == null) {
        ...
        analytics.sendPaymentErrorWithDetailsEvent(
            ...
            BillingAnalytics.PAYMENT_METHOD_APPC,
            transactionBuilder.type,
            payment.errorCode.toString(),
            payment.errorMessage.toString())
      }
    }
  }
  ...
  private fun handleFraudFlow() {
    disposables.add(
        onChainBuyInteract
            ...
            .flatMap { blocked ->
              if (blocked) {
                onChainBuyInteract.isWalletVerified().observeOn(viewScheduler).doOnSuccess {
                    verified ->
                  if (verified) {
                ...
                Single.just(true).observeOn(viewScheduler).doOnSuccess {
  ...
  private fun isError(status: Payment.Status): Boolean {
    return status == Payment.Status.ERROR ||
        status == Payment.Status.NO_FUNDS ||
        status == Payment.Status.NONCE_ERROR ||
        status == Payment.Status.NO_ETHER ||
        status == Payment.Status.NO_INTERNET ||
        status == Payment.Status.NO_TOKENS ||
        status == Payment.Status.NETWORK_ERROR ||
        status == Payment.Status.FORBIDDEN ||
