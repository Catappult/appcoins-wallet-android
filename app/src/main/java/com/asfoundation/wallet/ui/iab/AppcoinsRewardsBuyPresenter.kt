...
class AppcoinsRewardsBuyPresenter(
  ...
  private fun handleOkErrorClick() {
    disposables.add(
        view
            .getOkErrorClick()
            .doOnNext { view.showPaymentMethods() }
            .subscribe(
                {},
                {
        ...
        rewardsManager
            ...
            .andThen(
                rewardsManager.getPaymentStatus(
                    packageName, transactionBuilder.skuId, transactionBuilder.amount()))
    ...
    return when (transaction.status) {
      ...
      Status.COMPLETED -> {
        if (isBds && isManagedPaymentType(transactionBuilder.type)) {
          ...
          rewardsManager
              .getTransaction(packageName, sku, amount)
              .firstOrError()
              .flatMapCompletable { transactionModel ->
                Completable.fromAction { view.showTransactionCompleted() }
                    ...
                    .andThen(
                        Completable.fromAction {
                          view.finish(transactionModel.txId, transactionModel.txId ?: "")
                        })
              }
        }
      }
  ...
  private fun handleFraudFlow() {
    disposables.add(
        appcoinsRewardsBuyInteract
            ...
            .flatMap { blocked ->
              if (blocked) {
                appcoinsRewardsBuyInteract.isWalletVerified().observeOn(viewScheduler).doOnSuccess {
                  if (it) {
  ...
  fun sendPaymentSuccessEvent(txId: String) {
    paymentAnalytics.stopTimingForPurchaseEvent(
        PaymentMethodsAnalytics.PAYMENT_METHOD_APPC, true, isPreSelected)
  ...
  private fun isErrorStatus(status: Status): Boolean =
      status === Status.ERROR ||
          ...
          status === Status.SUB_ALREADY_OWNED
  ...
  private fun isManagedPaymentType(type: String): Boolean =
      type == BillingSupportedType.INAPP.name ||
          type == BillingSupportedType.INAPP_SUBSCRIPTION.name
}
