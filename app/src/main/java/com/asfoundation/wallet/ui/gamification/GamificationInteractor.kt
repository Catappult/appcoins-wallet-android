...
constructor(
  ...
  fun getLevels(offlineFirst: Boolean = true): Observable<Levels> {
    return defaultWallet.find().flatMapObservable {
      gamification.getLevels(it.address, offlineFirst)
    }
  }

  fun getUserStats(): Observable<PromotionsGamificationStats> {
    return getCurrentPromoCodeUseCase().flatMapObservable { promoCode ->
      defaultWallet.find().flatMapObservable {
  ...
  fun getUserLevel(): Single<Int> {
    return getCurrentPromoCodeUseCase().flatMap { promoCode ->
      defaultWallet.find().flatMap { gamification.getUserLevel(it.address, promoCode.code) }
    }
  }
  ...
  fun getEarningBonus(
    ...
    return defaultWallet
        ...
        .flatMap { wallet: Wallet ->
          gamification
              .getEarningBonus(wallet.address, packageName, amount, promoCodeString, currency)
              .map { forecastBonus ->
  ...
  fun hasNewLevel(
    ...
    return if (gamificationResponse == null ||
        gamificationResponse.status != PromotionsResponse.Status.ACTIVE) {
      Single.just(false)
    } else {
  ...
  fun levelShown(level: Int, gamificationContext: GamificationContext): Completable {
    return defaultWallet.find().flatMapCompletable {
      gamification.levelShown(it.address, level, gamificationContext)
    }
  }
  ...
  fun getAppcToLocalFiat(
    ...
    return conversionService.getAppcToLocalFiat(value, scale, getFromCache).onErrorReturn {
      FiatValue(BigDecimal("-1"), "", "")
    }
  }
  ...
  fun isBonusActiveAndValid(forecastBonus: ForecastBonusAndLevel): Boolean {
    return forecastBonus.status == ForecastBonus.Status.ACTIVE &&
        forecastBonus.amount > BigDecimal.ZERO
  }
}
