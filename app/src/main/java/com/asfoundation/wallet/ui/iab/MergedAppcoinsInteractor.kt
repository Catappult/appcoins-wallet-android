...
class MergedAppcoinsInteractor
@Inject
constructor(
  ...
  fun retrieveAppcAvailability(
    ...
    return if (isSubscription) {
      ...
      inAppPurchaseInteractor.getBalanceState(transactionBuilder).map {
        when (it) {
          InAppPurchaseService.BalanceState.NO_ETHER ->
              Availability(false, R.string.purchase_no_eth_body)
          InAppPurchaseService.BalanceState.NO_TOKEN,
          InAppPurchaseService.BalanceState.NO_ETHER_NO_TOKEN ->
              Availability(false, R.string.purchase_no_appcoins_body)
