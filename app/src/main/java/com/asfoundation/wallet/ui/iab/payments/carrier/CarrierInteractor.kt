...
constructor(
    ...
    private val walletVerificationInteractor:
        com.appcoins.wallet.feature.walletInfo.data.verification.WalletVerificationInteractor,
    private val getCurrentPromoCodeUseCase:
  ...
  private fun getTransactionBuilder(transactionData: String): Single<TransactionBuilder> {
    return inAppPurchaseInteractor
        .parseTransaction(transactionData, true)
  ...
  private fun getAddresses(packageName: String): Single<WalletAddresses> {
    return Single.zip(
        ...
        partnerAddressService.getAttribution(packageName).subscribeOn(rxSchedulers.io)) {
            addressModel,
            attributionEntity ->
          WalletAddresses(
              addressModel.address,
              addressModel.signedAddress,
              attributionEntity.oemId,
  ...
  fun getWalletStatus(): Single<WalletStatus> {
    return Single.zip(
        walletBlockedInteract.isWalletBlocked().subscribeOn(rxSchedulers.io),
        isWalletVerified().subscribeOn(rxSchedulers.io),
