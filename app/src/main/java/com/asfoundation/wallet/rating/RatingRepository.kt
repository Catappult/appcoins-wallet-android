...
constructor(
   ...
   * - hasSeenDialog should return true as soon as the entry fragment is displayed for the first
   *   time
   * - This methood should only return true once the rating flow is seen and closed for the first
   *   time
   */
    ...
    val body =
        WalletFeedbackBody(
            WalletFeedbackBody.Ticket(
                "Wallet Feedback - $walletAddress", WalletFeedbackBody.Comment(feedbackText)))
    return retrofitZendeskNetworkApi
        .sendFeedback("Basic ${BuildConfig.FEEDBACK_ZENDESK_API_KEY}", body)
