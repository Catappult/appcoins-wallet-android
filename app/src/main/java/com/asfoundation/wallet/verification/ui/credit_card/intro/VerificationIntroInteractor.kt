...
constructor(
  ...
  fun loadVerificationIntroModel(): Single<VerificationIntroModel> {
    return getVerificationInfo().flatMap {
      loadPaymentMethodInfo(it.currency, it.value).map { paymentInfoModel ->
        mapToVerificationIntroModel(it, paymentInfoModel)
      }
    }
  }
  ...
  private fun mapToVerificationIntroModel(
      infoModel: VerificationInfoModel,
  ...
  fun showSupport(): Completable {
    return Completable.fromAction { supportInteractor.displayChatScreen() }
  }
}
