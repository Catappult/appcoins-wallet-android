...
class AdyenPaymentPresenter(
  ...
  private fun handleForgetStoredCardClick() {
    disposables.add(
        view
            ...
            .observeOn(viewScheduler)
            .doOnNext { success ->
              if (success) view.restartFragment() else view.showGenericError()
            }
            .subscribe(
                {},
                {
              ...
              if (it.error.hasError) {
                ...
                when (paymentType) {
                  PaymentType.CARD.name -> {
                    ...
                    paymentAnalytics.stopTimingForTotalEvent(
  ...
  private fun handleBuyClick(priceAmount: BigDecimal, priceCurrency: String) {
    disposables.add(
        view
            ...
            .flatMapSingle {
              paymentAnalytics.startTimingForPurchaseEvent()
              view.retrievePaymentData().firstOrError()
            }
            .observeOn(viewScheduler)
            .doOnNext {
      ...
      when {
                  ...
                  isPaymentFailed(it.status) -> {
                    if (paymentModel.status == FAILED && paymentType == PaymentType.PAYPAL.name) {
                      ...
                      Completable.fromAction {
                            ...
                            sendPaymentErrorEvent(it.error.errorInfo?.httpCode, errorDetails)
        ...
        paymentModel.error.hasError ->
            Completable.fromAction {
              ...
              sendPaymentErrorEvent(paymentModel.error.errorInfo?.httpCode, errorDetails)
              handleErrors(paymentModel.error, paymentModel.refusalCode)
            }
  ...
  private fun handleSuccessTransaction(purchaseBundleModel: PurchaseBundleModel): Completable =
      Completable.fromAction { view.showSuccess(purchaseBundleModel.renewal) }
          .andThen(
              Completable.timer(view.getAnimationDuration(), TimeUnit.MILLISECONDS, viewScheduler))
  ...
  private fun handleFraudFlow(@StringRes error: Int, fraudCheckIds: List<Int>) {
    disposables.add(
        adyenPaymentInteractor
            .isWalletVerified()
            .observeOn(viewScheduler)
            .doOnSuccess { verified -> view.showVerificationError(verified) }
  ...
  // This method is used to avoid the nameValuePairs key problem that occurs when we pass a
  // JSONObject trough a GSON converter
  ...
  private fun handleMorePaymentsStoredClick() {
    disposables.add(
        view
            .getMorePaymentMethodsStoredClicks()
            .observeOn(networkScheduler)
            .doOnNext { handleMorePaymentsAnalytics(transactionBuilder) }
            .observeOn(viewScheduler)
            .doOnNext { showMoreMethods() }
  ...
  private fun handleErrors(error: Error, code: Int? = null) {
    when {
      ...
      error.errorInfo?.errorType == ErrorType.CARD_SECURITY_VALIDATION ->
      ...
      error.errorInfo?.errorType == ErrorType.INVALID_COUNTRY_CODE ->
          view.showSpecificError(R.string.unknown_error)
      error.errorInfo?.errorType == ErrorType.PAYMENT_NOT_SUPPORTED_ON_COUNTRY ->
          view.showSpecificError(R.string.purchase_error_payment_rejected)
      error.errorInfo?.errorType == ErrorType.CURRENCY_NOT_SUPPORTED ->
      ...
      error.errorInfo?.errorType == ErrorType.TRANSACTION_AMOUNT_EXCEEDED ->
