...
constructor(
    ...
    return ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMap { ewt ->
      sandboxApi
          ...
          .map { response: SandboxResponse ->
            SandboxTransaction(response.uid, response.hash, response.status, response.mapValidity())
          }
          .onErrorReturn {
  ...
  private fun handleCreateTransactionErrorCodes(
    ...
    return SandboxTransaction(null, null, null, validity, errorCode.toString(), errorContent ?: "")
  }
}
