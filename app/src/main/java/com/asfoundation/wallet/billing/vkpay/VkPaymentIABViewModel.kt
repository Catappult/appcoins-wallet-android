...
data class VkPaymentIABState(val vkTransaction: Async<VkPayTransaction> = Async.Uninitialized) :
    ViewState

@HiltViewModel
class VkPaymentIABViewModel
@Inject
constructor(
...
) : BaseViewModel<VkPaymentIABState, VkPaymentIABSideEffect>(VkPaymentIABState()) {
          ...
          .map {
            when (it.status) {
              ...
              Transaction.Status.FRAUD -> {
                stopTransactionStatusTimer()
                sendSideEffect {
                  VkPaymentIABSideEffect.ShowError(R.string.purchase_error_wallet_block_code_403)
                }
              }
