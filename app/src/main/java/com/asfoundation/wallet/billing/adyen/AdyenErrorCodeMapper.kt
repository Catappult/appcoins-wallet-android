...
class AdyenErrorCodeMapper @Inject constructor() {
  ...
  internal fun map(errorCode: Int): Int {
    return when (errorCode) {
      ...
      ISSUER_SUSPECTED_FRAUD -> R.string.purchase_card_error_general_2
      REFERRAL,
      ACQUIRER_ERROR,
      ISSUER_UNAVAILABLE -> R.string.purchase_card_error_general_1
      EXPIRED_CARD -> R.string.purchase_card_error_expired
      INVALID_AMOUNT,
      NOT_ENOUGH_BALANCE,
      WITHDRAW_AMOUNT_EXCEEDED,
      RESTRICTED_CARD -> R.string.purchase_card_error_no_funds
      INVALID_CARD_NUMBER -> R.string.purchase_card_error_invalid_details
      NOT_SUPPORTED -> R.string.purchase_card_error_not_supported
      INCORRECT_ONLINE_PIN,
      PIN_TRIES_EXCEEDED -> R.string.purchase_error_wrong_pin_body
      NOT_3D_AUTHENTICATED -> R.string.purchase_error_3d_body
      FRAUD,
      CANCELLED_DUE_TO_FRAUD -> R.string.purchase_card_error_general_2
