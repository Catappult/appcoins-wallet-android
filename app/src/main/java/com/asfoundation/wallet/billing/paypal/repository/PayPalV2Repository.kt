...
constructor(
    ...
    return ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMap { ewt ->
      paypalV2Api
          ...
          .map { response: PaypalV2StartResponse ->
            PaypalTransaction(response.uid, response.hash, response.status, response.mapValidity())
          }
          .onErrorReturn {
    ...
    return ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMap { ewt ->
      paypalV2Api
          .createToken(
              ...
              authorization = ewt,
              createTokenRequest =
                  CreateTokenRequest(Urls(returnUrl = returnUrl, cancelUrl = cancelUrl)))
          .map { response: PaypalV2CreateTokenResponse -> PaypalCreateToken.map(response) }
    }
  }

  fun createBillingAgreement(walletAddress: String, token: String): Single<PaypalCreateAgreement> {
    return ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMap { ewt ->
      paypalV2Api
          .createBillingAgreement(walletAddress = walletAddress, authorization = ewt, token = token)
          .map { response: PaypalV2CreateAgreementResponse -> PaypalCreateAgreement.map(response) }
    }
  }

  fun cancelToken(walletAddress: String, token: String): Completable {
    return ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMapCompletable { ewt
      ->
      paypalV2Api
          .cancelToken(walletAddress = walletAddress, authorization = ewt, token = token)
          .ignoreElement()
    }
  }
  ...
  fun getCurrentBillingAgreement(
    ...
    return ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMap { ewt ->
      paypalV2Api
          .getCurrentBillingAgreement(walletAddress = walletAddress, authorization = ewt)
          .map { response: PaypalV2GetAgreementResponse -> response.uid.isNotEmpty() }
          .onErrorReturn { false }
    }
  }

  fun removeBillingAgreement(walletAddress: String): Completable {
    return ewtObtainer.getEwtAuthentication().subscribeOn(rxSchedulers.io).flatMapCompletable { ewt
      ->
      paypalV2Api
          .removeBillingAgreement(walletAddress = walletAddress, authorization = ewt)
          .ignoreElement()
    }
  }
  ...
  private fun handleCreateTransactionErrorCodes(
    ...
    return PaypalTransaction(null, null, null, validity, errorCode.toString(), errorContent ?: "")
  }
}
