...
@AndroidEntryPoint
class VkPaymentIABFragment :
    BasePageViewFragment(), SingleStateFragment<VkPaymentIABState, VkPaymentIABSideEffect> {
  ...
  private fun setupTransactionCompleteAnimation() {
    ...
    binding.successContainerVk.lottieTransactionSuccess.addAnimatorListener(
    ...
    binding.successContainerVk.lottieTransactionSuccess.setFontAssetDelegate(
  ...
  fun showError() {
    ...
    clearVkPayCheckout()
    binding.errorTryAgainVk.setOnClickListener { iabView.navigateBack() }
    binding.errorCancelVk.setOnClickListener { iabView.close(bundle = null) }
  }
  ...
  private fun showSuccessAnimation() {
    viewModel.sendPaymentSuccessEvent(
        requireArguments().getParcelable(TRANSACTION_DATA_KEY)!!, viewModel.transactionUid!!)
    ...
    val bundle =
        Bundle().apply {
          ...
          putString(
              ...
              (requireArguments().getSerializable(AMOUNT_KEY) as BigDecimal).toString())
          putString(
              VkPaymentTopUpFragment.TOP_UP_CURRENCY, requireArguments().getString(CURRENCY_KEY))
