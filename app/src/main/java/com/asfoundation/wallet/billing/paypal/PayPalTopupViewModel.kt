...
constructor(
  ...
  fun startPayment(
    ...
    topUpAnalytics.sendConfirmationEvent(
        amount.toDouble(), "top_up", PaymentMethodsAnalytics.PAYMENT_METHOD_PP_V2)
    attemptTransaction(
  ...
  fun attemptTransaction(createTokenIfNeeded: Boolean = true, amount: String, currency: String) {
    compositeDisposable.add(
        createPaypalTransactionTopupUseCase(value = amount, currency = currency)
                ...
                PaypalTransaction.PaypalValidityState.NO_BILLING_AGREEMENT -> {
                  ...
                  if (createTokenIfNeeded) {
                    ...
                    topUpAnalytics.sendPaypalErrorEvent(
                        errorCode = it.errorCode, errorDetails = it.errorMessage ?: "")
  ...
  fun startBillingAgreement(amount: String, currency: String) {
      ...
      compositeDisposable.add(
          createPaypalAgreementUseCase(authenticatedToken)
              ...
              .doOnSuccess {
                ...
                attemptTransaction(createTokenIfNeeded = false, amount, currency)
              }
              .subscribe(
                  {},
                  {
                    Log.d(TAG, it.toString())
                    topUpAnalytics.sendPaypalErrorEvent(
                        errorDetails = "Paypal BillingAgreement error")
                    _state.postValue(State.Error(R.string.unknown_error))
                  }))
    }
  }
  ...
  fun cancelToken() {
    authenticatedToken?.let {
      cancelPaypalTokenUseCase
          .invoke(it)
          .subscribeOn(networkScheduler)
          .observeOn(viewScheduler)
          .subscribe { _state.postValue(State.TokenCanceled) }
    }
  }
        ...
        waitForSuccessPaypalUseCase(uid ?: "")
            ...
            .subscribe(
                ...
                },
                {
                  ...
                  topUpAnalytics.sendPaypalErrorEvent(
                      errorDetails = "Error on Settled transaction polling")
                }))
  }
