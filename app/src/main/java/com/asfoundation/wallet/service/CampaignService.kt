...
class CampaignService @Inject constructor(
  ...
  fun submitProof(proof: Proof, wallet: String): Single<String> {
    return campaignApi.submitProof(
      SerializedProof(
        proof.campaignId, proof.packageName, wallet, proof.proofComponentList,
  ...
  fun getCampaign(
    address: String, packageName: String,
  ...
  private fun handleResponse(response: PoaInformationResponse): PoaInformationModel {
    return PoaInformationModel(
      response.remainingPoa, response.hoursRemaining,
  ...
  interface CampaignApi {
    ...
    fun submitProof(
      @Body body: SerializedProof?, @Query("version_code")
      versionCode: Int
    ): Observable<SubmitPoAResponse>
    ...
    fun getCampaign(
      ...
      @Query("vercode") versionCode: Int
    ): Observable<GetCampaignResponse>
  }
}

class SerializedProof(
  val bid_id: String?,
...
data class Campaign(
  val campaignId: String, val campaignStatus: CampaignStatus,
