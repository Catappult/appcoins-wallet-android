...
class MyWalletsFragment : BasePageViewFragment(),
  ...
  override fun onStateChanged(state: MyWalletsState) {
    ...
    when (val asyncValue = state.walletVerifiedAsync) {
      ...
      is Async.Success -> asyncValue().run {
        when (status) {
          com.appcoins.wallet.feature.walletInfo.data.verification.BalanceVerificationStatus.VERIFIED -> showVerified(
            false
          )

          com.appcoins.wallet.feature.walletInfo.data.verification.BalanceVerificationStatus.UNVERIFIED -> showUnverified(
            false
          )

          com.appcoins.wallet.feature.walletInfo.data.verification.BalanceVerificationStatus.CODE_REQUESTED -> showUnverifiedInsertCode(
            false
          )
