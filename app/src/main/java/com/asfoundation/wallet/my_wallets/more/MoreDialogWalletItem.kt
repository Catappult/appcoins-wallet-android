...
private val MoreDialogStateItem.checkMarkVisibility
  get() = if (isSelected) VISIBLE else INVISIBLE
...
private fun Context.ItemCard(onTap: ((View) -> Unit)?, child: View) =
    MaterialCardView(this).apply {
      layoutParams =
          MarginLayoutParams(MATCH_PARENT, WRAP_CONTENT).apply { topMargin = resources.dpToPx(8) }
      elevation = 0F
    ...
    TextView(this).apply {
      layoutParams =
          LinearLayoutCompat.LayoutParams(
              resources.dpSizeToPx(width), resources.dpSizeToPx(height), weight)
...
private fun Resources.dpSizeToPx(value: Int): Int =
    when (value) {
      WRAP_CONTENT,
      MATCH_PARENT -> value
      else -> dpToPx(value)
