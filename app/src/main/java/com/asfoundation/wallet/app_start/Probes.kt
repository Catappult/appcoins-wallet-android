...
class AppStartProbe @Inject constructor(private val analyticsManager: AnalyticsManager) {
  ...
  operator fun invoke(startMode: StartMode) {
    ...
    val data =
        when (startMode) {
          ...
          is StartMode.RestoreGuestWalletFlow -> mapOf(BACKUP to startMode.backup)
          else -> mapOf(PACKAGE_NAME to "", INTEGRATION_FLOW to "other", SOURCE to "", SKU to "")
        }
    analyticsManager.logEvent(data, FIRST_LAUNCH, AnalyticsManager.Action.OPEN, WALLET)
  }
