...
class GPInstallUseCaseImpl
@Inject
constructor(private val repository: GooglePlayInstallRepository) : GPInstallUseCase {
  override suspend operator fun invoke(): StartMode.GPInstall? {
    val referrer = withTimeoutOrNull(5000) { repository.getReferrerUrl() }?.splitQuery()
