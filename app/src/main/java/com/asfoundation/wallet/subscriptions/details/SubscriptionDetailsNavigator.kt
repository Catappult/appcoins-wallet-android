...
class SubscriptionDetailsNavigator
@Inject
constructor(private val fragmentManager: FragmentManager) {
  ...
  fun showCancelSubscription(subscriptionItem: SubscriptionItem, sharedElement: View) {
    ...
    val bottomSheet =
        SubscriptionCancelFragment.newInstance(subscriptionItem, transitionName)
  ...
  fun showRenewSuccess() {
    val bottomSheet =
        SubscriptionSuccessFragment.newInstance(
            SubscriptionSuccessFragment.SubscriptionSuccess.RENEW) as BottomSheetDialogFragment
