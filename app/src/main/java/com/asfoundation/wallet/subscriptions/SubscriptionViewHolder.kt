...
class SubscriptionViewHolder(itemView: View, private val currencyFormatUtils: CurrencyFormatUtils) :
  ...
  fun bind(
      ...
      clickCallback: PublishSubject<Pair<SubscriptionItem, View>>?,
      position: Int
  ) {
    itemView.apply {
  ...
  private fun showToExpireInfo(view: View, item: SubscriptionItem) {
    ...
    item.expiry?.let {
      binding.expiresOn.text =
          view.context.getString(R.string.subscriptions_expiration_body, dateFormat.format(it))
    }
  }
}
