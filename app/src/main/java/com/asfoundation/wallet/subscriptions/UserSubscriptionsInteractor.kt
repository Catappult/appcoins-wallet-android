...
class UserSubscriptionsInteractor
@Inject
constructor(
  ...
  fun loadSubscriptions(freshReload: Boolean): Observable<SubscriptionModel> {
    return walletService.getWalletAddress().flatMapObservable {
      userSubscriptionRepository.getUserSubscriptions(it, freshReload)
    }
  }

  fun cancelSubscription(packageName: String, uid: String): Completable {
    return walletService.getAndSignCurrentWalletAddress().flatMapCompletable {
