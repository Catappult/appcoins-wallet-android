...
@Inject
constructor(private val api: ExperimentsApi, private val featureFlagsDao: FeatureFlagsDao) :
  ...
  override suspend fun getFeatureFlag(
    ...
    return try {
      api.assignExperiment(experimentId = flagId, userId = userId, body = UserProfile(profileData))
          .toFeatureFlag()
          .also {
  ...
  override suspend fun sendImpression(flagId: String, userId: String) =
      try {
        api.sendEvent(flagId, userId, Events(events = listOf(Event(name = IMPRESSION_EVENT))))
      } catch (_: Throwable) {}

  override suspend fun sendAction(flagId: String, userId: String, name: String, payload: String) {
    require(name != IMPRESSION_EVENT) {
      "$IMPRESSION_EVENT is reserved. Use recordImpression() for impressions"
    }
    try {
      api.sendEvent(
          flagId, userId, Events(events = listOf(element = Event(name = name, payload = payload))))
    } catch (_: Throwable) {}
  }
  ...
  private fun ApiAssignment.toFeatureFlag(): FeatureFlag? =
      when (status) {
        ...
        Status.EXISTING_ASSIGNMENT ->
            FeatureFlag(isNew = false, variant = assignment, payload = payload)
