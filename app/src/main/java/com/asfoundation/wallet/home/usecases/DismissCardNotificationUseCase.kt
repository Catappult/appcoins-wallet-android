...
class DismissCardNotificationUseCase
@Inject
constructor(
  ...
  private fun referralDismiss(referralNotification: ReferralNotification): Completable {
    return findDefaultWalletUseCase().flatMapCompletable {
  ...
  private fun autoUpdateDismiss(): Completable {
    return autoUpdateRepository.loadAutoUpdateModel(false).flatMapCompletable {
      Completable.fromCallable {
