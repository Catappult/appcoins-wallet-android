...
class ObserveDefaultWalletUseCase
@Inject
constructor(private val walletRepository: WalletRepositoryType) {

  operator fun invoke(): Observable<Wallet> {
    return walletRepository.observeDefaultWallet().subscribeOn(Schedulers.io()).onErrorResumeNext {
        _: Throwable ->
      return@onErrorResumeNext walletRepository
          .fetchWallets()
