...
class GetCardNotificationsUseCase
@Inject
constructor(
  ...
  operator fun invoke(): Single<List<CardNotification>> {
    val getUnwatchedPendingBonusNotification =
        referralInteractor.getUnwatchedPendingBonusNotification().subscribeOn(Schedulers.io())
    val getUnwatchedUpdateNotification =
        getUnwatchedUpdateNotificationUseCase().subscribeOn(Schedulers.io())
    val getUnwatchedPromotionNotification =
        promotionsInteractor.getUnwatchedPromotionNotification().subscribeOn(Schedulers.io())
    return Single.zip(
        getUnwatchedPendingBonusNotification,
        getUnwatchedUpdateNotification,
        getUnwatchedPromotionNotification) {
            referralNotification: CardNotification,
            updateNotification: CardNotification,
            promotionNotification: CardNotification ->
