...
constructor(
  ...
  fun getAvailableAmount(ewt: String): Single<BigDecimal> {
    return withdrawApi.getAvailableAmount(ewt).subscribeOn(schedulers.io).map { it.amount }
  }
  ...
  fun withdrawAppcCredits(ewt: String, email: String, amount: BigDecimal): Single<WithdrawResult> {
    ...
    return withdrawApi
        .withdrawAppcCredits(ewt, WithdrawBody(email, amount))
  ...
  fun getStoredUserEmail(): Single<String> {
    return Single.fromCallable {
      return@fromCallable withdrawLocalStorage.getUserEmail()
          ?: throw RuntimeException("Couldn't find user e-mail.")
    }
  }
}
