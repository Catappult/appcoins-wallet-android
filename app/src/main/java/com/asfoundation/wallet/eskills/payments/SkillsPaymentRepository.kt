...
constructor(
  ...
  override fun getBalance(): Single<BigDecimal> {
    return getWalletInfoUseCase(null, cached = false).subscribeOn(schedulers.io).map {
      it.walletBalance.creditsBalance.token.amount
    }
  }

  override fun getLocalFiatAmount(value: BigDecimal, currency: String): Single<Price> {
    return currencyConversionService
        .getLocalFiatAmount(value.toString(), currency)
  ...
  override fun getFiatToAppcAmount(value: BigDecimal, currency: String): Single<Price> {
    return currencyConversionService
        .getFiatToAppcAmount(value.toString(), currency)
        .map { Price(it.amount, it.currency, it.symbol) }
        .subscribeOn(schedulers.io)
  }

  override fun getFormattedAppcAmount(value: BigDecimal, currency: String): Single<String> {
    return getFiatToAppcAmount(value, currency).map {
      currencyFormatUtils.formatCurrency(it.amount, WalletCurrency.APPCOINS)
    }
  }
  ...
  override fun pay(
    ...
    return appCoinsCreditsPayment.pay(eskillsPaymentData, ticket).subscribeOn(schedulers.io)
  }

  override fun sendUserToVerificationFlow(context: Context) {
    val intent =
        VerificationCreditCardActivity.newIntent(context).apply {
