...
import com.asf.wallet.ui.widget.OnDepositClickListener;

import static com.asf.wallet.C.CHANGELLY_REF_ID;
import static com.asf.wallet.C.COINBASE_WIDGET_CODE;
import static com.asf.wallet.C.ETH_SYMBOL;
import static com.asf.wallet.C.SHAPESHIFT_KEY;
...
public class DepositView extends FrameLayout implements View.OnClickListener {
  ...
  private static final Uri changellyteUri =
      Uri.parse("https://changelly.com/widget/v1?auth=email&from=BTC")
          .buildUpon()
    ...
    switch (v.getId()) {
      case R.id.action_shapeshift: {
        uri = shapeshiftUri.buildUpon()
            .appendQueryParameter("destination", wallet.address)
            .build();
      }
      break;
      case R.id.action_changelly: {
        uri = changellyteUri.buildUpon()
            .appendQueryParameter("address", wallet.address)
            .build();
      }
      break;
      default:
