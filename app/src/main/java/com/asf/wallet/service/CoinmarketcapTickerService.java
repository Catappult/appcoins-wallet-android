...
public class CoinmarketcapTickerService implements TickerService {
  ...
  private void buildApiClient(String baseUrl) {
    coinmarketApiClient = new Retrofit.Builder().baseUrl(baseUrl)
        .client(httpClient)
  ...
  @Override public Observable<Ticker> fetchTickerPrice(String ticker) {
    return coinmarketApiClient.fetchTickerPrice(ticker)
        .lift(apiError(gson))
        .map(r -> r[0])
  ...
  public interface CoinmarketApiClient {
    @GET("/v1/ticker/{ticker}") Observable<Response<Ticker[]>> fetchTickerPrice(
        @Path("ticker") String ticker);
  }

  private final static class ApiErrorOperator<T> implements ObservableOperator<T, Response<T>> {

    @Override public Observer<? super Response<T>> apply(Observer<? super T> observer)
        throws Exception {
