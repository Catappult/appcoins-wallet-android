...
public class TokenHolder extends BinderViewHolder<Token> implements View.OnClickListener {
  ...
  @Override public void bind(@Nullable Token data, @NonNull Bundle addition) {
    ...
    try {
      ...
      symbol.setText(TextUtils.isEmpty(token.tokenInfo.name) ? token.tokenInfo.symbol
  ...
  private void fillCurrency(BigDecimal ethBalance, TokenTicker ticker) {
    String converted = ethBalance.compareTo(BigDecimal.ZERO) == 0 ? EMPTY_BALANCE
    ...
    String lbl =
        getString(R.string.token_balance, ethBalance.compareTo(BigDecimal.ZERO) == 0 ? "" : "$",
            converted, formattedPercents);
    Spannable spannable = new SpannableString(lbl);
    spannable.setSpan(new ForegroundColorSpan(color), converted.length() + 1, lbl.length(),
