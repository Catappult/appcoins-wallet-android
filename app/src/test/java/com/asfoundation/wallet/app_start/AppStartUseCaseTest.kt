...
/**
 * AS a Wallet Developer, I WANT to track modes of app start, FOR analysing newcomers and altering
 * the first screen
 *
 * Since it is impossible to know if app data was cleared or not we assume that if it is not a fresh
 * install then this is not the first run. So the first run event will occur only if app was not
 * updated yet and run count is 0
 */
...
internal class AppStartUseCaseTest {
  ...
  fun `Listen before job started`(data: TestData) = coScenario { scope ->
    m Given
        "use case with the given repository, GPInstallUseCase and PendingPurchaseFlowUseCase mocks"
    val useCase =
        AppStartUseCase(
  ...
  companion object {
    ...
    fun testDataProvider(): List<TestData> =
        listOf(
            TestData(
                scenario = "App started -> First", givenData = GivenData(), thenData = ThenData()),
            TestData(
                ...
                givenData =
                    GivenData(
                        PendingPurchaseFlowUseCase =
                            PendingPurchaseFlowUseCaseMock(pendingPurchase())),
            ...
            TestData(
                scenario =
                    "App started after update with Pending Purchase and GP Install -> Subsequent",
                givenData =
                    GivenData(
                        repository = AppStartRepositoryMock(updatedAfter = 5.days()),
                        PendingPurchaseFlowUseCase =
            ...
            TestData(
                scenario =
                    "App started for the second time with Pending Purchase and GP Install -> Subsequent",
                givenData =
                    GivenData(
                        repository = AppStartRepositoryMock(runCount = 1),
                        PendingPurchaseFlowUseCase =
            ...
            TestData(
                scenario =
                    "App started for the second time after update with Pending Purchase -> Subsequent",
                givenData =
                    GivenData(
                        repository = AppStartRepositoryMock(runCount = 1, updatedAfter = 5.days()),
                        PendingPurchaseFlowUseCase =
            ...
            TestData(
                scenario =
                    "App started for the second time after update with GP Install -> Subsequent",
                givenData =
                    GivenData(
                        repository = AppStartRepositoryMock(runCount = 1, updatedAfter = 5.days()),
                        GPInstallUseCase = GPInstallUseCaseMock(gpInstall())),
            ...
            TestData(
                scenario =
                    "App started for the second time after update with Pending Purchase and GP Install -> Subsequent",
                givenData =
                    GivenData(
                        repository = AppStartRepositoryMock(runCount = 1, updatedAfter = 5.days()),
                        PendingPurchaseFlowUseCase =
...
class GPInstallUseCaseMock(private val GPInstall: StartMode.GPInstall? = null) : GPInstallUseCase {
