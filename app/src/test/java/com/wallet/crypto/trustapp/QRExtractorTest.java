...
public class QRExtractorTest {
  ...
  @Test public void extractingIsCorrect() throws Exception {

    QRURLParser parser = QRURLParser.getInstance();

    // Correct string
    String extractedString = parser.extractAddressFromQrString(
        "ethereum:0x0000000000000000000000000000000000000000?value=0");
    assertTrue("0x0000000000000000000000000000000000000000".equals(extractedString));

    // Protocol with spaces
    extractedString = parser.extractAddressFromQrString(
        "NG node:0x0000000000000000000000000000000000000000?value=0");
    assertTrue("0x0000000000000000000000000000000000000000".equals(extractedString));

    // Protocol with upper case
    extractedString = parser.extractAddressFromQrString(
        "PROTOCOL:0x0000000000000000000000000000000000000000?value=0");
    assertTrue("0x0000000000000000000000000000000000000000".equals(extractedString));

    // String without value
    extractedString =
        parser.extractAddressFromQrString("ethereum:0x0000000000000000000000000000000000000000");
    assertTrue("0x0000000000000000000000000000000000000000".equals(extractedString));

    // Lowed case
    extractedString =
        parser.extractAddressFromQrString("ethereum:0xabcdef0000000000000000000000000000000000");
    assertTrue("0xabcdef0000000000000000000000000000000000".equals(extractedString));

    // Upper case
    extractedString =
        parser.extractAddressFromQrString("ethereum:0xABC0000000000000000000000000000000000000");
    ...
    extractedString =
        parser.extractAddressFromQrString("0x0000000000000000000000000000000000000000");
    assertTrue("0x0000000000000000000000000000000000000000".equals(extractedString));

    // Address with value
    extractedString =
        parser.extractAddressFromQrString("0x0000000000000000000000000000000000000000?value=0");
    assertTrue("0x0000000000000000000000000000000000000000".equals(extractedString));

    // Address with a different protocol
    extractedString =
        parser.extractAddressFromQrString("OMG:0x0000000000000000000000000000000000000000");
    assertTrue("0x0000000000000000000000000000000000000000".equals(extractedString));

    // Address longer than expected, parse out an address anyway
    extractedString =
        parser.extractAddressFromQrString("0x0000000000000000000000000000000000000000123");
    assertTrue("0x0000000000000000000000000000000000000000".equals(extractedString));

    // Two parameters
    extractedString = parser.extractAddressFromQrString(
        "notethereum:0x0000000000000000000000000000000000000abc?value=0&symbol=USD");
    assertTrue("0x0000000000000000000000000000000000000abc".equals(extractedString));

    // Parse out address even with protocol missing
    extractedString = parser.extractAddressFromQrString(
        ":0x0000000000000000000000000000000000000abc?value=0invalid");
    assertTrue("0x0000000000000000000000000000000000000abc".equals(extractedString));

    // Two query parameters
    extractedString = parser.extractAddressFromQrString(
        "ethereum:0x0000000000000000000000000000000000000123?key=value&key=value");
    ...
    extractedString = parser.extractAddressFromQrString(
        "ethereum:0x0000000000000000000000000000000000000XyZ?value=0invalid");
    assertTrue("0x0000000000000000000000000000000000000xyz".equals(extractedString));

    // Negative: null when address too short
    extractedString =
        parser.extractAddressFromQrString("ethereum:0x0000000000000000abc?value=0invalid");
    assertTrue(extractedString == null);
  }

  @Test public void parseQRURLTest() {
    QRURLParser parser = QRURLParser.getInstance();
