...
@RunWith(MockitoJUnitRunner.class) public class AirdropTest {
  ...
  @Test public void requestSuccess() {
    when(airdropService.requestAirdrop(WALLET_ADDRESS, CHAIN_ID)).thenReturn(Single.just(
    ...
    TestObserver<AirdropData> test = airdrop.getStatus()
        .test();
    airdrop.request(WALLET_ADDRESS, CHAIN_ID);
  ...
  @Test public void requestApiFail() {
    when(airdropService.requestAirdrop(WALLET_ADDRESS, CHAIN_ID)).thenReturn(Single.just(
        new AirdropService.AirDropResponse(AirdropService.Status.FAIL, null, null, -1,
            ERROR_MESSAGE)));
    TestObserver<AirdropData> test = airdrop.getStatus()
        .test();
    airdrop.request(WALLET_ADDRESS, CHAIN_ID);
  ...
  @Test public void requestFail() {
    when(airdropService.requestAirdrop(WALLET_ADDRESS, CHAIN_ID)).thenReturn(
