...
class BrokerVerificationPreferencesDataSource
@Inject
constructor(private val sharedPreferences: SharedPreferences) {
  fun saveVerificationStatus(walletAddress: String, verificationStatusId: Int) =
      sharedPreferences.edit().putInt(WALLET_VERIFIED + walletAddress, verificationStatusId).apply()
  ...
  fun removeCachedWalletValidationStatus(walletAddress: String) =
      sharedPreferences.edit().remove(WALLET_VERIFIED + walletAddress).apply()
