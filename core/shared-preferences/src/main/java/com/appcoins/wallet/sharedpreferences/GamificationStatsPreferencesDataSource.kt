...
class GamificationStatsPreferencesDataSource
@Inject
constructor(private val sharedPreferences: SharedPreferences) {
  ...
  fun saveShownLevel(key: String, level: Int) = sharedPreferences.edit().putInt(key, level).apply()
  ...
  fun setSeenGenericPromotion(genericKey: String) =
      sharedPreferences.edit().putBoolean(genericKey, true).apply()

  fun setGamificationLevel(gamificationLevel: Int) =
      sharedPreferences.edit().putInt(GAMIFICATION_LEVEL, gamificationLevel).apply()
  ...
  fun setGamificationDisclaimerShown() =
      sharedPreferences.edit().putBoolean(SHOW_GAMIFICATION_DISCLAIMER, false).apply()

  fun setSeenWalletOrigin(wallet: String, walletOrigin: String) =
      sharedPreferences.edit().putString(WALLET_ORIGIN + wallet, walletOrigin).apply()
  ...
  fun setVipCalloutAlreadySeen(wallet: String, isSeen: Boolean) =
      sharedPreferences.edit().putBoolean(VIP_CALLOUT_SEEN + wallet, isSeen).apply()
  ...
  fun setReferralNotificationSeen(wallet: String, isSeen: Boolean) =
      sharedPreferences.edit().putBoolean(REFERRAL_NOTIFIC_SEEN + wallet, isSeen).apply()
}
