...
class FingerprintPreferencesDataSource
@Inject
constructor(private val sharedPreferences: SharedPreferences) {
  ...
  fun setAuthenticationPermission(result: Boolean) =
      sharedPreferences.edit().putBoolean(AUTHENTICATION_PERMISSION, result).apply()
  ...
  fun setAuthenticationErrorTime(timer: Long) =
      sharedPreferences.edit().putLong(AUTHENTICATION_ERROR_TIME, timer).apply()
  ...
  fun setSeenFingerprintTooltip() =
      sharedPreferences.edit().putBoolean(HAS_SEEN_FINGERPRINT_TOOLTIP, true).apply()
}
