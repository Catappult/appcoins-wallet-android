...
class SupportPreferencesDataSource
@Inject
constructor(private val sharedPreferences: SharedPreferences) {
  ...
  fun updateUnreadConversations(unreadConversations: Int) =
      sharedPreferences.edit().putInt(UNREAD_CONVERSATIONS, unreadConversations).apply()

  fun resetUnreadConversations() = sharedPreferences.edit().putInt(UNREAD_CONVERSATIONS, 0).apply()
}
