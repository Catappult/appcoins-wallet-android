...
abstract class BaseActivity : AppCompatActivity(), ActivityResultSharer {
  ...
  /**
   * Testing the functionality of the base activity without this 2 functions protected Toolbar
   * toolbar() { Toolbar toolbar = findViewById(R.id.toolbar); toolbar.setVisibility(View.VISIBLE);
   * if (toolbar != null) { setSupportActionBar(toolbar); toolbar.setTitle(getTitle()); }
   * enableDisplayHomeAsUp(); return toolbar; }
   *
   * protected void setCollapsingTitle(String title) { CollapsingToolbarLayout collapsing =
   * findViewById(R.id.toolbar_layout); if (collapsing != null) { collapsing.setTitle(title); } }
   */
  ...
  override fun onOptionsItemSelected(item: MenuItem): Boolean {
    if (item.itemId == R.id.home) {
      KeyboardUtils.hideKeyboard(window.decorView.rootView)
      finish()
    }
    return true
  }
