...
abstract class BaseViewModel<S : ViewState, E : SideEffect>(initialState: S) : ViewModel() {
   ...
   *   that is set when [Async.Loading] is emitted. It's useful if you want to display previously
   *   successful data on loading or fail.
  ...
  protected fun <T> Observable<T>.asAsyncToState(
    ...
    return doOnNext { value ->
          setState { reducer(Async.Success(value, retainValue?.get(this)?.value)) }
        }
        .doOnError { e ->
          setState { reducer(Async.Fail(e.mapToError(), retainValue?.get(this)?.value)) }
        }
  }
   ...
   * [reducer]. It sends [Async.Loading] immediately once called and maps to [Async.Success] when
   * completed. If an exception is thrown, it sends [Async.Fail].
   *
   * @param retainValue A state property that will be retained for [Async.Loading] and [Async.Fail]
   *   that is set when [Async.Loading] is emitted. It's useful if you want to display previously
   *   successful data on loading or fail.
  ...
  protected fun Completable.asAsyncToState(
    ...
    setState { reducer(Async.Loading(retainValue?.get(this)?.value)) }
    return doOnComplete { setState { reducer(Async.Success(Unit, retainValue?.get(this)?.value)) } }
        .doOnError { e ->
          setState { reducer(Async.Fail(e.mapToError(), retainValue?.get(this)?.value)) }
        }
  }
   ...
   * success and failure.
   *
   * @param retainValue A state property that will be retained for [Async.Loading] that is set when
   *   [Async.Loading] is emitted. It's useful if you want to display previously successful data on
   *   loading.
   * @param reducer A reducer that takes the current state and returns the new state. A common
   *   implementation of this with a data class is: `{ copy(stateProperty = it) }`.
   */
   ...
   * source (e.g. if we want to "refresh" an Observable). An alternative would be to use a "refresh
   * emitting" observable source and use switchMap, however that wouldn't work as nicely with
   * [asAsyncToState].
   *
   * Also subscribes safely to this view model scope. Once the ViewModel is cleared, this
   * subscription is also cleared.
   *
   * @param id Unique identifier for this subscription. Make sure you use different ids for
   *   different streams.
   * @param onErrorAction Action block that receives a [Throwable] and is executed at the end of the
   *   stream if it isn't caught until then.
   */
  protected fun <T> Observable<T>.repeatableScopedSubscribe(
    ...
    return repeatableSubscriptionMap[id]!!.apply { compositeDisposable.add(this) }
  }

  /**
   * Subscribes to the Single source cancelling a previous subscription for the [id], if it exists.
   * The main purpose of this is to avoid concurrent subscriptions of the same Single source (e.g.
   * if we want to "refresh" a Single). An alternative would be to use a "refresh emitting"
   * observable source and use switchMap, however that wouldn't work as nicely with
   * [asAsyncToState].
   *
   * Also subscribes safely to this view model scope. Once the ViewModel is cleared, this
   * subscription is also cleared.
   ...
   * source (e.g. if we want to "refresh" a Completable). An alternative would be to use a "refresh
   * emitting" observable source and use switchMap, however that wouldn't work as nicely with
   * [asAsyncToState].
   *
   * Also subscribes safely to this view model scope. Once the ViewModel is cleared, this
  ...
  /**
   * Subscribes safely to this view model scope. Once the ViewModel is cleared, this subscription is
   * also cleared.
   */
  protected fun <T> Observable<T>.scopedSubscribe(
    ...
    return disposable.apply { compositeDisposable.add(this) }
  }
  ...
  protected fun <T> Single<T>.scopedSubscribe(
    ...
    val disposable =
        subscribe(
            { if (onSuccessAction != null) onSuccessAction(it) }, { onErrorAction?.invoke(it) })
    return disposable.apply { compositeDisposable.add(this) }
  }
