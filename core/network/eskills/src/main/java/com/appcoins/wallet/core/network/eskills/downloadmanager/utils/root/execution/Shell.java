 ...
 */
package com.appcoins.wallet.core.network.eskills.downloadmanager.utils.root.execution;
...
public class Shell {
            ...
            if (fields.length >= 2 && fields[1] != null) {
              ...
              if (id == totalRead) {
                ...
                while (command.totalOutput > command.totalOutputProcessed) {
                  ...
                  if (iterations == 0) {
                    iterations++;
                    RootShell.log("Waiting for output to be processed. "
                        + command.totalOutputProcessed
                        + " Of "
                        + command.totalOutput);
                  }
  ...
  private Shell(String cmd, ShellType shellType, ShellContext shellContext, int shellTimeout)
    ...
    if (this.shellContext == ShellContext.NORMAL) {
      ...
      if (isSELinuxEnforcing() && (display != null) && (internal != null) && (display.endsWith(
          "SUPERSU")) && (Integer.valueOf(internal) >= 190)) {
    ...
    this.inputStream =
        new BufferedReader(new InputStreamReader(this.proc.getInputStream(), "UTF-8"));
    this.errorStream =
        new BufferedReader(new InputStreamReader(this.proc.getErrorStream(), "UTF-8"));
  ...
  public static Shell startRootShell(int timeout, ShellContext shellContext, int retry)
      throws IOException, TimeoutException, RootDeniedException {
    ...
    if (Shell.rootShell == null) {
      ...
      try {
        RootShell.log("Context is different than open shell, switching context... "
            + Shell.rootShell.shellContext
            + " VS "
            + shellContext);
  ...
  public String getCommandQueuePositionString(Command cmd) {
    return "Command is in position "
        + getCommandQueuePosition(cmd)
        + " currently executing command at position "
        + this.write
        + " and the number of commands is "
        + commands.size();
  }
