...
class DynamicSplitsMapper {
  ...
  fun mapDynamicSplits(
    serverDynamicSplitsList: List<DynamicSplitsResponse.DynamicSplit>
  ): List<DynamicSplit> {

    return serverDynamicSplitsList.map {
      DynamicSplit(
        it.name, it.type, it.md5sum, it.path, it.filesize, it.deliveryTypes,
  ...
  private fun mapDynamicConfigSplits(
    splits: List<com.appcoins.wallet.core.network.eskills.aab.v7.Split>
  ): List<Split> {
