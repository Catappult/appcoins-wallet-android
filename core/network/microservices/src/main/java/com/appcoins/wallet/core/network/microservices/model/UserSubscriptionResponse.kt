 ...
 * @param renewal Date when subscription is renewed, null when subStatus is PENDING, PAUSED, EXPIRED
 *   or REVOKED
 * @param expiry Date when subscription will expire or has expired, null when subStatus is PENDING
 *   or PAUSED
 * @param ended Date when subscription ended, null when subStatus is PENDING, ACTIVE, GRACE, HOLD,
 *   CANCELED or PAUSED
 */
data class UserSubscriptionResponse(
    ...
    val title: String,
    val period: String,
    @SerializedName("substatus") val subStatus: SubscriptionSubStatus,
    val started: String?,
    val renewal: String?,
    val expiry: String?,
    val ended: String?,
