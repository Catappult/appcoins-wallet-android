...
interface SubscriptionBillingApi {
   ...
   * @param skus (optionnal) list of SKUs to filter by. up to 100, which may be set as a comma
   *   separated list of values, or as a JSON or query string array.
   * @param limit (optionnal) The limit on the maximum number of items to return per page, up
   *   to 100.
   * @param currency (optionnal) The preferred currency to return prices in, as an ISO 4217
   *   alphabetic code.
   */
   ...
   * @param currency The preferred currency to generate the in-app subscription token with, as an
