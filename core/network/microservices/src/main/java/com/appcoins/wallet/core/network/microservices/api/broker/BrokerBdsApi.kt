...
interface BrokerBdsApi {
   ...
   * @param currencyType, filter for appc and credits payment, use fiat if you don't want appc and
   *   credits
   * @param direct, either if it returns non-direct payments (false) (earn appcoins and ask someone
   *   to pay) or not
   * @param type, INAPP, INAPP_UNMANAGED or TOPUP. This is used to filter async payments in INAPP
   *   and INAPP_UNMANAGED, if null no filter is applied by transactionType
   */
  ...
  /**
   * All optional fields should be passed despite possible being null as these are required by some
   * applications to complete the purchase flow
   *
   * @param gateway type of the transaction that is being created;
   ...
   * @param token
   * @param developerPayload Group of details used in some purchases by the application to complete
   *   the purchase
