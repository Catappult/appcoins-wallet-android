...
internal fun ApplicationDefaultConfig.buildConfigFields(project: Project, rootDir: File) {
  ...
  for (variable in defaultBuildConfigList) {
    buildConfigField(
        ...
        name = variable.name,
        value =
            if (variable.value != null) project.property(variable.value).toString()
...
fun ApplicationDefaultConfig.handleOemAndStoreAddresses(rootDir: File) {
  ...
  buildConfigField(
      "String", "DEFAULT_OEM_ADDRESS", "\"" + (json["oems"] as Map<*, *>)["default_address"] + "\"")
  buildConfigField(
      "String",
