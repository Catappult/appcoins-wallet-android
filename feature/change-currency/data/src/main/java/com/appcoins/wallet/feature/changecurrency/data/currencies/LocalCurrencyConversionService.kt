...
constructor(
  ...
  fun getAppcToLocalFiat(
    ...
    return if (getFromCache) {
      currencyConversionRatesPersistence.getAppcToLocalFiat(value, scale)
        ...
        getValueToFiat(value, "APPC", null, scale).flatMap {
          currencyConversionRatesPersistence
              .saveRateFromAppcToFiat(value, it.amount.toString(), it.currency, it.symbol)
  ...
  fun getEtherToLocalFiat(value: String, scale: Int): Single<FiatValue> {
    return getValueToFiat(value, "ETH", null, scale).flatMap {
      currencyConversionRatesPersistence
          .saveRateFromEthToFiat(value, it.amount.toString(), it.currency, it.symbol)
          .andThen(Single.just(it))
          .onErrorReturn { throwable: Throwable ->
            throwable.printStackTrace()
            it
  ...
  fun getFiatToAppc(currency: String, value: String, scale: Int): Single<FiatValue> {
    return tokenToLocalFiatApi.convertFiatToAppc(currency, value).map {
        response: ConversionResponseBody ->
      FiatValue(
          response.value.setScale(scale, RoundingMode.FLOOR), response.currency, response.sign)
    }
  }
  ...
  fun getValueToFiat(
    ...
    val api =
        if (targetCurrency != null)
            tokenToLocalFiatApi.getValueToTargetFiat(currency, value, targetCurrency)
  ...
  fun getFiatToLocalFiat(currency: String, value: String, scale: Int): Single<FiatValue> {
    return tokenToLocalFiatApi.getValueToTargetFiat(currency, value).map {
        response: ConversionResponseBody ->
      FiatValue(
          response.value.setScale(scale, RoundingMode.FLOOR), response.currency, response.sign)
    }
