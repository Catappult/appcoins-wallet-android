...
class RoomCurrencyConversionRatesPersistence(
  ...
  override fun getAppcToLocalFiat(appcValue: String, scale: Int): Single<FiatValue> {
    return currencyConversionRatesDao.getRate(APPC).map { map(it, appcValue, scale) }
  }

  override fun getEthToLocalFiat(ethValue: String, scale: Int): Single<FiatValue> {
    return currencyConversionRatesDao.getRate(ETH).map { map(it, ethValue, scale) }
  }

  private fun map(entity: CurrencyConversionRateEntity, value: String, scale: Int): FiatValue {
    return FiatValue(applyRate(entity.rate, value, scale), entity.fiatCurrency, entity.fiatSymbol)
  }

  /**
   * Make sure that the rate being saved does not invalidate what may already be saved in the
   * database. This is particularly relevant for the case where we save an empty rate. If the new
   * rate is non-empty (non-zero), then it will always be saved on DB. If there is a non-zero rate
   * already saved, and the fiat currency is the same, with the new rate being zero, then the new
   * rate will not be saved. If the fiat currency changes (e.g. user starts a VPN connection to a
   * location with a different currency), then the new rate is always saved (even if it is empty).
  ...
  private fun saveRate(
    ...
    return currencyConversionRatesDao
        .getRate(currencyFrom)
        .map { shouldSaveNewRate(it, newRateEntity) }
        .onErrorReturn { true }
  ...
  private fun calculateRate(fromValue: String, toValue: String): String {
    ...
    if (fromValueDecimal.compareTo(BigDecimal.ZERO) == 0 ||
        toValueDecimal.compareTo(BigDecimal.ZERO) == 0)
        return ZERO_RATE
    return toValueDecimal.divide(fromValueDecimal, RATE_SCALE, RATE_ROUNDING).toString()
  }

  private fun applyRate(rate: String, value: String, scale: Int): BigDecimal {
    return BigDecimal(rate).multiply(BigDecimal(value)).setScale(scale, RATE_ROUNDING)
  }
