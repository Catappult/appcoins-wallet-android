...
constructor(
  ...
  fun getLatestWalletInfo(walletAddress: String): Single<WalletInfo> =
      updateWalletInfo(walletAddress).andThen(observeWalletInfo(walletAddress).firstOrError())

  /**
   * Note that this will fetch from network if no cached value exists, so it is not a true cached
   * value for every case. If you always want to fetch from network, see [observeUpdatedWalletInfo]
   */
  fun getCachedWalletInfo(walletAddress: String): Single<WalletInfo> =
      walletInfoDao.getWalletInfo(walletAddress).flatMap { list ->
        if (list.isNotEmpty()) {
          Single.just(list[0].toWalletInfo())
        } else {
          fetchWalletInfo(walletAddress).map { it.toWalletInfo() }
        }
      }

  fun observeUpdatedWalletInfo(walletAddress: String): Observable<WalletInfo> =
      Observable.merge(
          observeWalletInfo(walletAddress), updateWalletInfo(walletAddress).toObservable())

  fun observeWalletInfo(walletAddress: String): Observable<WalletInfo> =
      walletInfoDao
          .observeWalletInfo(walletAddress.normalize())
          .map { it.toWalletInfo() }
  ...
  fun updateWalletInfo(walletAddress: String): Completable =
      fetchWalletInfo(walletAddress).ignoreElement().onErrorComplete().subscribeOn(rxSchedulers.io)
  ...
  private fun fetchWalletInfo(walletAddress: String): Single<WalletInfoEntity> {
        ...
        .flatMap { currency ->
          api.getWalletInfo(walletAddress, currency.get())
              .flatMap { walletInfoResponse ->
                ...
                balanceRepository
                    .getWalletBalance(walletInfoResponse)
