...
constructor(
  ...
  override fun getWalletAddress(): Single<String> = find().map { toChecksumAddress(it.address) }
      ...
      find()
          ...
          .onErrorResumeNext { _: Throwable ->
            Observable.just(WalletGetterStatus.CREATING.toString())
                .mergeWith(
                    walletCreatorInteract.create("Main Wallet").map { it.address }.toObservable())
          }

  override fun signContent(content: String): Single<String> =
      find().flatMap { wallet ->
        getPrivateKey(wallet).map { sign(normalizer.normalize(content), it) }
      }
  ...
  override fun signSpecificWalletAddressContent(
      ...
      walletRepository.findWallet(walletAddress).flatMap { wallet ->
        getPrivateKey(wallet).map { sign(normalizer.normalize(content), it) }
      }
  ...
  private fun find(): Single<Wallet> =
      walletRepository.getDefaultWallet().onErrorResumeNext {
        walletRepository
            .fetchWallets()
  ...
  private fun getPrivateKey(wallet: Wallet): Single<ECKey> =
      if (stringECKeyPair?.first?.equals(wallet.address, true) == true) {
        ...
        passwordStore
            .getPassword(wallet.address)
            .flatMap { password ->
              accountKeyService.exportAccount(wallet.address, password, password).map { json ->
  ...
  fun getAddressFromPrivateKey(key: String): Single<String> {
    ...
    return passwordStore.generatePassword().map {
      val addressIncompl = org.web3j.crypto.Wallet.create(it, keypair, N_VALUE, P_VALUE).address
      "0x$addressIncompl"
    }
  }
}

enum class WalletGetterStatus {
  CREATING
}
