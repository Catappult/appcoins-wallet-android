...
class BalanceRepository @Inject constructor() {
  ...
  fun getWalletBalance(info: WalletInfoResponse): Single<WalletBalance> {
    val credits =
        BalanceUtils.weiToEth(info.appcCreditsBalanceWei.toBigDecimal())
            .setScale(FIAT_SCALE, RoundingMode.FLOOR)
    val appc =
        BalanceUtils.weiToEth(info.appcBalanceWei.toBigDecimal())
            .setScale(FIAT_SCALE, RoundingMode.FLOOR)
    val eth =
        BalanceUtils.weiToEth(info.ethBalanceWei.toBigDecimal())
            .setScale(FIAT_SCALE, RoundingMode.FLOOR)

    return Single.just(
        mapToWalletBalance(
            credits,
            info.appcCreditsBalancFiat.setScale(FIAT_SCALE, RoundingMode.FLOOR),
            appc,
            info.appcBalanceFiat.setScale(FIAT_SCALE, RoundingMode.FLOOR),
            eth,
            info.ethBalanceFiat.setScale(FIAT_SCALE, RoundingMode.FLOOR),
            info.currency,
            info.symbol))
  }

  fun mapToWalletBalance(
      creditsValue: BigDecimal,
      creditsFiatAmount: BigDecimal,
      appcValue: BigDecimal,
      appcFiatAmount: BigDecimal,
      ethValue: BigDecimal,
      ethFiatAmount: BigDecimal,
      fiatCurrency: String,
      fiatSymbol: String
  ): WalletBalance {
    val creditsToken =
