...
constructor(private val getWalletInfoUseCase: GetWalletInfoUseCase) {
  ...
  operator fun invoke(
    ...
    return getWalletInfoUseCase(address, cached = true).flatMap { walletInfo ->
      val scaledValue = Convert.toWei(value, unit)
      val scaledCredits =
          Convert.toWei(walletInfo.walletBalance.creditsBalance.token.amount, Convert.Unit.ETHER)
      val scaledAppc =
