...
constructor(
  ...
  override fun restoreKeystoreToWallet(
    ...
    return accountKeystoreService.restoreKeystore(store, password, newPassword).flatMap {
        restoreResult ->
      ...
      return@flatMap savePasswordCompletable.andThen(Single.just(restoreResult))
    }
  }
  ...
  override fun restorePrivateKeyToWallet(
    ...
    return accountKeystoreService.restorePrivateKey(privateKey, newPassword).flatMap { restoreResult
      ->
      var savePasswordCompletable = Completable.complete()
      if (restoreResult is SuccessfulRestore) {
        savePasswordCompletable = savePassword(restoreResult.address, newPassword)
  ...
  override fun savePassword(address: String, password: String): Completable {
    return passwordStore.setPassword(address, password).onErrorResumeNext { throwable ->
      deleteWallet(address, password).lift(Operators.completableErrorProxy(throwable))
    }
  }
}
